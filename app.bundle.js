(() => {
  ...
  function mergeWeightLogEntry(existing, incoming) {
    ...
    let changed = false;
    if (merged.weight === void 0 && incoming.weight !== void 0) {
      merged.weight = incoming.weight;
      if (incoming.weightSource) merged.weightSource = incoming.weightSource;
      changed = true;
    } else if (merged.weight !== void 0 && merged.weightSource === void 0 && incoming.weightSource) {
      merged.weightSource = incoming.weightSource;
      changed = true;
    }
    if (merged.bodyFat === void 0 && incoming.bodyFat !== void 0) {
      merged.bodyFat = incoming.bodyFat;
      if (incoming.bodyFatSource) merged.bodyFatSource = incoming.bodyFatSource;
      changed = true;
    } else if (merged.bodyFat !== void 0 && merged.bodyFatSource === void 0 && incoming.bodyFatSource) {
      merged.bodyFatSource = incoming.bodyFatSource;
      changed = true;
    }
    for (const key of Object.keys(incoming)) {
      if (["date", "weight", "weightSource", "bodyFat", "bodyFatSource"].includes(key)) continue;
          ...
          if (key === "weight-logs" && Array.isArray(data[key])) {
            ...
            for (const incoming of data[key]) {
              ...
              if (!existing) {
                ...
                const { merged: merged2 } = mergeWeightLogEntry(existing, incoming);
  ...
  function TodayTab({ today, viewDate, setViewDate, fitnessSettings, ziipSettings, todos, setTodos, weightLogs, meals, weeklyPool, todayPicks, setTodayPicks, poolDone, setPoolDone, dailyOrder, setDailyOrder, setTab }) {
  ...
  function WeightTab({ logs, setLogs }) {
      ...
      try {
        ...
        for (const e of allEntries) {
          ...
          if (typeof e.fat === "number") {
            entry.bodyFat = Math.round(e.fat * 10) / 10;
            entry.bodyFatSource = "fitbit";
          }
        ...
        const touchedDates = [];
        for (const incoming of Object.values(byDate)) {
          const existing = byDateExisting[incoming.date];
          if (!existing) {
            byDateExisting[incoming.date] = incoming;
            newDayCount++;
            ...
            const { merged: merged2, changed } = mergeWeightLogEntry(existing, incoming);
            if (changed) {
              byDateExisting[incoming.date] = merged2;
              enrichedDayCount++;
              touchedDates.push(incoming.date);
        ...
        if (enrichedDayCount) parts.push(`${enrichedDayCount} existing day(s) filled in`);
    ...
    const importRenphoCsv = async (file) => {
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (entry.bodyFat !== void 0) entry.bodyFatSource = "renpho";
    ...
    const add = () => {
      if (!weight) return;
      const entry = { date, weight: parseFloat(weight), weightSource: "fitbit" };
      if (bodyFat) {
        entry.bodyFat = parseFloat(bodyFat);
        entry.bodyFatSource = "fitbit";
      }
    ...
    const buildSourceSplitData = (field, sourceField) => {
      return trendLogs.filter((l) => l[field] !== void 0).map((l) => {
        const src = l[sourceField] || "manual";
        const point = { date: l.date.slice(5) };
        point[src] = l[field];
        return point;
      });
    };
    const weightTrendData = buildSourceSplitData("weight", "weightSource");
    const bodyFatTrendData = buildSourceSplitData("bodyFat", "bodyFatSource");
    const hasSource = (data, src) => data.some((d) => d[src] !== void 0);
    const [selectedMetric, setSelectedMetric] = useState(RENPHO_FIELDS[0].key);
    const selectedField = RENPHO_FIELDS.find((f) => f.key === selectedMetric);
    const metricTrendData = trendLogs.filter((l) => l[selectedMetric] !== void 0).map((l) => ({ date: l.date.slice(5), value: l[selectedMetric] }));
