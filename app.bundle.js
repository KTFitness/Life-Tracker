  ...
  function WeightTab({ logs, setLogs }) {
    ...
    const importRenphoCsv = async (file) => {
      ...
      try {
        ...
        if (newEntries.length === 0) {
          setRenphoResult("Nothing new \u2014 every date in this file is already in your log.");
          return;
        }
        const merged = [...logs, ...newEntries].sort((a, b) => a.date.localeCompare(b.date));
        setLogs(merged);
        const dates = newEntries.map((e) => e.date).sort();
        setRenphoResult(`Imported ${newEntries.length} new day(s), from ${fmtDate(dates[0])} to ${fmtDate(dates[dates.length - 1])}. Existing entries were left untouched.`);
      } catch (e) {
