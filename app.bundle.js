(() => {
  ...
  function MealsTab({ today, meals, setMeals, recipes, setRecipes, shoppingList, setShoppingList, nutritionTargets, setNutritionTargets }) {
    ...
    const [copySlot, setCopySlot] = useState("breakfast");
    ...
    const copyMealSlot = () => {
      ...
      const items = cloneItems(getSlotItems(sourceEntry, copySlot));
      setMeals({ ...meals, [day]: { ...entry, [copySlot + "Items"]: items, [copySlot]: mealSlotDisplayText(items, recipes) } });
      setCopyMessage(`Copied ${copySlot} from ${fmtDate(copyFromDate)}.`);
    };
    const copyEntireDay = () => {
      if (copyFromDate === day) return;
      const sourceEntry = meals[copyFromDate];
