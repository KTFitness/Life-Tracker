(() => {
  ...
  var GOOGLE_CLIENT_ID = "834932217321-ge8upsvk5jikj80gqk414s6s939hmoib.apps.googleusercontent.com";
  ...
  function App() {
    ...
    useEffect(() => {
      const setupClient = () => {
        if (window.google && window.google.accounts && window.google.accounts.oauth2 && !gcalTokenClientRef.current) {
          gcalTokenClientRef.current = window.google.accounts.oauth2.initTokenClient({
            ...
            callback: (response) => {
              if (response && response.access_token) {
                ...
                syncGoogleCalendar(response.access_token);
              } else {
                setGcalStatus("error");
                setGcalError("Google didn't return an access token.");
              }
            }
          });
        }
      };
      if (window.google) setupClient();
      else {
    ...
    const connectGoogleCalendar = () => {
      ...
      if (!gcalTokenClientRef.current) {
        setGcalStatus("error");
        setGcalError("Google sign-in hasn't loaded yet \u2014 try again in a moment.");
        return;
      }
      gcalTokenClientRef.current.requestAccessToken({ prompt: gcalTokenRef.current ? "" : "consent" });
    };
    const syncGoogleCalendar = async (tokenOverride) => {
      const token = tokenOverride || gcalTokenRef.current;
      if (!token) {
      ...
      try {
        ...
        const timeMax = new Date(Date.now() + 60 * 24 * 60 * 60 * 1e3).toISOString();
        ...
        if (res.status === 401) {
          ...
          setGcalError("Your Google connection expired \u2014 tap Connect again.");
          return;
        }
        ...
        const items = (data.items || []).map((ev) => {
          ...
          const timeLabel = ev.start.dateTime ? new Date(ev.start.dateTime).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : null;
          const text = timeLabel ? `${ev.summary || "(untitled)"}, ${timeLabel}` : ev.summary || "(untitled)";
          return { id: "gcal-" + ev.id, date, text, source: "calendar-live", done: false };
        }).filter(Boolean);
        setTodos((prev) => {
          const withoutOldGcal = (prev || []).filter((t) => t.source !== "calendar-live");
          return [...withoutOldGcal, ...items];
        });
        setGcalStatus("connected");
        setGcalLastSync((/* @__PURE__ */ new Date()).toISOString());
      } catch (e) {
        setGcalStatus("error");
        setGcalError(e && e.message ? e.message : String(e));
      }
    };
    ...
    const syncIcloudCalendar = async (urlOverride) => {
      ...
      try {
        ...
        const items = events.filter((ev) => ev.date && ev.date >= todayStr && ev.date <= maxDateStr).map((ev) => {
          ...
          return { id: "ical-" + (ev.uid || ev.date + "-" + (ev.summary || "")), date: ev.date, text: text2, source: "calendar-live", done: false };
        });
        setTodos((prev) => {
  ...
  function TodoTab({ todos, setTodos, weeklyPool, setWeeklyPool, todayPicks, setTodayPicks, dismissedIds, setDismissedIds, today, gcalStatus, gcalError, gcalLastSync, connectGoogleCalendar, syncGoogleCalendar, icloudUrlInput, setIcloudUrlInput, icloudStatus, icloudError, icloudLastSync, saveIcloudUrl, syncIcloudCalendar }) {
    ...
    const syncCalendar = () => {
      ...
      const nonCalendarTodos = todos.filter((t) => t.source !== "calendar" && t.source !== "calendar-live" && t.source !== "retreat");
    ...
    })), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Reads your real Google Calendar (read-only) for the next 60 days. The connection only lasts about an hour at a time \u2014 reconnect whenever you want a fresh sync." }, "Google Calendar"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" } }, gcalStatus === "disconnected" || gcalStatus === "error" ? /* @__PURE__ */ React.createElement(Btn, { onClick: connectGoogleCalendar }, "Connect Google Calendar") : /* @__PURE__ */ React.createElement(Btn, { onClick: () => syncGoogleCalendar(), disabled: gcalStatus === "syncing" }, gcalStatus === "syncing" ? "Syncing\u2026" : "Sync now"), gcalStatus === "connected" && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.green } }, "\u2713 Connected"), gcalStatus === "connecting" && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "Waiting for Google sign-in\u2026")), gcalLastSync && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, fontSize: 11.5, color: COLORS.inkSoft } }, "Last synced: ", new Date(gcalLastSync).toLocaleString()), gcalError && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, fontSize: 12.5, color: COLORS.danger } }, gcalError)), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Paste the public read-only link from iPhone Calendar \u2192 Calendars \u2192 (i) next to your calendar \u2192 Share Link. Anyone with this exact link can view it, so keep it private." }, "iCloud Calendar"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 } }, /* @__PURE__ */ React.createElement(
      Input,
      {
