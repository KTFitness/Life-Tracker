(() => {
  ...
  function MealsTab({ today, meals, setMeals, recipes, setRecipes, shoppingList, setShoppingList, nutritionTargets, setNutritionTargets }) {
    ...
    const applyBulkMealFill = () => {
      ...
      if (!text) return;
      const idField = bulkSlot + "RecipeId";
      const servingsField = bulkSlot + "Servings";
      const updates = {};
      for (let i = 0; i < n; i++) {
        const d = addDaysISO(bulkStartDate, i);
        const existing = meals[d] || { breakfast: "", breakfastRecipeId: null, breakfastServings: 1, lunch: "", lunchRecipeId: null, lunchServings: 1, dinner: "", dinnerRecipeId: null, dinnerServings: 1, snacks: "", snacksRecipeId: null, snacksServings: 1, notes: "" };
        updates[d] = {
          ...existing,
          [bulkSlot]: text,
          [idField]: recipe ? recipe.id : null,
          [servingsField]: recipe ? parseFloat(bulkServings) || 1 : existing[servingsField]
        };
      }
