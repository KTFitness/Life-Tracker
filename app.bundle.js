(() => {
  ...
  function WeightTab({ logs: rawLogs, setLogs }) {
    ...
    const importFitbitWeightFiles = async (fileList) => {
      ...
      try {
        ...
        const { result, newCount, enrichedCount, touchedDates } = applySourceImport(logs, byId);
    ...
    const importRenphoCsv = async (file) => {
      ...
      try {
        ...
        const { result, newCount, enrichedCount, touchedDates } = applySourceImport(logs, parsed);
        if (newCount === 0 && enrichedCount === 0) {
    ...
    )), fitbitError && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, fontSize: 13, color: COLORS.danger } }, fitbitError), fitbitResult && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, fontSize: 13, color: COLORS.green, fontWeight: 600 } }, "\u2713 ", fitbitResult)), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Now shows a separate row per source when you've logged both RENPHO and Fitbit on the same day." }, "History"), [...logs].reverse().map((l) => {
  ...
  function TodoTab({ todos, setTodos, weeklyPool, setWeeklyPool, todayPicks, setTodayPicks, dismissedIds, setDismissedIds, today, icloudUrlInput, setIcloudUrlInput, icloudStatus, icloudError, icloudLastSync, saveIcloudUrl, syncIcloudCalendar, corsProxyUrlInput, setCorsProxyUrlInput, saveCorsProxyUrl }) {
    ...
    const upcoming = [...todos].filter((t) => t.source !== "calendar" && t.source !== "calendar-live" && t.source !== "retreat").sort((a, b) => a.date.localeCompare(b.date));
    ...
    const staleCalendarCount = todos.filter((t) => t.source === "calendar").length;
