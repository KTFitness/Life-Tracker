(() => {
  ...
  var POOL_CATEGORIES = [
    { key: "beauty", label: "Beauty & Hair", color: "plum" },
    { key: "cleaning", label: "Cleaning", color: "clay" },
    { key: "finances", label: "Finances", color: "slate" },
  ...
  function categoryOf(p) {
    return p.category || (p.id.startsWith("sew-") ? "sewing" : p.id.startsWith("life-") ? "life" : "other");
  }
  function poolCategoryLabel(p) {
    const cat = POOL_CATEGORIES.find((c) => c.key === categoryOf(p));
    return cat ? cat.label : "Uncategorised";
  }
  var TIME_BLOCK_CATEGORIES = [
    { key: "claude", label: "Claude / tracker", color: "#5C6B73" },
    { key: "day_planning", label: "Day planning", color: "#6C7A89" },
    { key: "dogs", label: "Dog care", color: "#5B7C8C" },
    { key: "driving", label: "Driving", color: "#7D8570" },
    { key: "exercise", label: "Exercise", color: "#6E7F4E" },
    { key: "admin_finance", label: "Finance admin", color: "#2E5266" },
    { key: "cleaning", label: "House cleaning", color: "#8A7B6C" },
    { key: "laundry", label: "Laundry", color: "#7FA3A8" },
    { key: "admin_life", label: "Life admin", color: "#52606D" },
    { key: "meal_planning", label: "Meal planning", color: "#8B9E6B" },
    { key: "meal_prep", label: "Meal prep", color: "#B08D57" },
    { key: "meals", label: "Meals", color: "#C9A961" },
    { key: "medical", label: "Medical appointment", color: "#A0453A" },
    { key: "other", label: "Other / custom", color: "#6B6459" },
    { key: "purchase_research", label: "Purchase research", color: "#9C6644" },
    { key: "read", label: "Reading", color: "#6B5B7A" },
    { key: "screen", label: "Screen time", color: "#B9714B" },
    { key: "sewing_tb", label: "Sewing", color: "#A63A50" },
    { key: "shopping", label: "Shopping", color: "#9C6644" },
    { key: "skincare", label: "Shower & skincare", color: "#C88EA7" },
    { key: "sleep", label: "Sleep", color: "#8B8680" },
    { key: "sleep_hygiene", label: "Sleep hygiene", color: "#9B8FA6" },
    { key: "social", label: "Social", color: "#D9A05B" },
    { key: "todo_general", label: "To-do list items", color: "#8C7A6B" },
    { key: "travel_planning", label: "Travel planning", color: "#4E8098" },
    { key: "unaccounted", label: "Unaccounted time", color: "#9B9B93" },
    { key: "unproductive", label: "Unproductive time", color: "#B5A28C" },
    { key: "vet", label: "Vet appointment", color: "#C1666B" },
    { key: "wake", label: "Wake", color: "#E0C097" },
    { key: "walk", label: "Walk", color: "#3E5641" }
  ];
  function timeBlockCategoryOf(key) {
    return TIME_BLOCK_CATEGORIES.find((c) => c.key === key) || null;
  }
  function generateHourMarks(dayStart, dayEnd) {
    const marks = [];
    let h = parseInt((dayStart || "06:00").split(":")[0], 10);
    const endH = parseInt((dayEnd || "22:00").split(":")[0], 10);
    let guard = 0;
    while (h < endH && guard < 48) {
  ...
  function getSlotsForHour(hourMark, splitLevel) {
    const h = hourMark.slice(0, 2);
    if (splitLevel === 4) return [0, 15, 30, 45].map((m) => `${h}:${String(m).padStart(2, "0")}`);
    if (splitLevel === 2) return [0, 30].map((m) => `${h}:${String(m).padStart(2, "0")}`);
    return [`${h}:00`];
  }
  function minimalSplitForTime(timeStr) {
    const mins = parseInt(timeStr.slice(3), 10);
    if (mins === 0) return 1;
    if (mins === 30) return 2;
    return 4;
  }
  function computeHourSplit(hourMark, dayEntry, morningTemplate, eveningTemplate, calendarBlocks) {
    const explicit = dayEntry && dayEntry.hourSplits && dayEntry.hourSplits[hourMark];
    if (explicit) return explicit;
    const hourPrefix = hourMark.slice(0, 2) + ":";
    let needed = 1;
    const consider = (timeStr) => {
      if (!timeStr || !timeStr.startsWith(hourPrefix)) return;
      needed = Math.max(needed, minimalSplitForTime(timeStr));
    };
    Object.keys(dayEntry && dayEntry.planned || {}).forEach(consider);
    Object.keys(dayEntry && dayEntry.actual || {}).forEach(consider);
    [...morningTemplate || [], ...eveningTemplate || []].forEach((t) => consider(t.time));
    (calendarBlocks || []).forEach((c) => consider(c.time));
    return needed;
  }
  function apptSortKey(text) {
    const m = /^(\d{2}:\d{2})/.exec(text || "");
    return m ? m[1] : "00:00";
  }
  function calendarEventsForDate(dateStr, todos) {
    const results = [];
    (todos || []).filter((t) => t.date === dateStr && (t.source === "calendar" || t.source === "calendar-live")).forEach((t) => {
      const text = t.text || "";
      const rangeMatch = /^(\d{2}:\d{2})-(\d{2}:\d{2}) — (.*)$/.exec(text);
      if (rangeMatch) {
        const [, start, end, title] = rangeMatch;
        const [sh, sm] = start.split(":").map(Number);
        const [eh, em] = end.split(":").map(Number);
        let curMin = sh * 60 + sm;
        const endMin = eh * 60 + em;
        let guard = 0;
        while (curMin < endMin && guard < 96) {
          const hourStartMin = Math.floor(curMin / 60) * 60;
          const hourEndMin = hourStartMin + 60;
          const segmentEnd = Math.min(endMin, hourEndMin);
          if (curMin === hourStartMin && segmentEnd === hourEndMin) {
            results.push({ time: `${String(Math.floor(curMin / 60)).padStart(2, "0")}:00`, title });
            curMin = hourEndMin;
          } else {
            let stepMin = curMin;
            while (stepMin < segmentEnd && guard < 96) {
              results.push({ time: `${String(Math.floor(stepMin / 60)).padStart(2, "0")}:${String(stepMin % 60).padStart(2, "0")}`, title });
              stepMin += 15;
              guard++;
            }
            curMin = segmentEnd;
      ...
      const singleMatch = /^(\d{2}:\d{2}) — (.*)$/.exec(text);
      if (singleMatch) results.push({ time: singleMatch[1], title: singleMatch[2] });
    });
    return results;
  }
  function getEffectivePlanned(dayEntry, morningTemplate, eveningTemplate, calendarBlocks) {
    const planned = {};
    [...morningTemplate || [], ...eveningTemplate || []].forEach((t) => {
      planned[t.time] = { category: t.category, label: t.label || "" };
    });
    (calendarBlocks || []).forEach((c) => {
      planned[c.time] = { category: "other", label: c.title };
    });
    Object.entries(dayEntry && dayEntry.planned || {}).forEach(([time, block]) => {
      planned[time] = block;
    });
    return planned;
  }
  function timeBlockDisplayLabel(block) {
    if (!block) return null;
    if (block.category === "other" && block.label) return block.label;
    const cat = timeBlockCategoryOf(block.category);
    return block.label || (cat ? cat.label : null);
  }
  function blocksMatch(a, b) {
    if (!a || !b || !a.category || !b.category) return false;
    if (a.category !== b.category) return false;
    if (a.category === "other") return (a.label || "").trim().toLowerCase() === (b.label || "").trim().toLowerCase();
    return true;
  }
  function generateScheduleSummaryText(dateISO, plannedMap, actualMap, notes, slots) {
    let text = `Schedule for ${dateISO}

`;
    text += "Time    Planned                        Actual\n";
    text += "----    -------                        ------\n";
    slots.forEach((t) => {
      const pLabel = timeBlockDisplayLabel(plannedMap[t]) || "\u2014";
      const aLabel = timeBlockDisplayLabel(actualMap[t]) || "\u2014";
      text += `${t}   ${pLabel.padEnd(30)} ${aLabel}
`;
    });
    text += `
Notes: ${notes || "(none)"}`;
  ...
  var BLOCKS = [
    ...
    {
      name: "Block 2 \u2014 Cycling Priority",
      weeks: "Weeks 7\u201312",
      days: [
        "__CYCLING_BLOCK2__",
        "__IRON_BLOCK2_FULLBODY__",
        "__CYCLING_BLOCK2__",
        "__PILATES_ROTATION__",
        "__CYCLING_BLOCK2__",
        "__IRON_BLOCK2_FULLBODY__",
    ...
    },
    {
      ...
      weeks: "Weeks 22\u201327",
      days: [
        "Pilates \u2014 trial app rotation (see Kirsty's plan)",
        "Iron \u2014 Full Body maintenance (40 min)",
        "Pilates \u2014 trial app rotation (see Kirsty's plan)",
  ...
  var SELF_CALENDAR_EMAILS = ["ktassell@bigpond.com", "kirstytassell@outlook.com"];
  function parseICS(text, windowStartISO, windowEndISO) {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith("END:VEVENT")) {
        if (cur) {
          if (!cur.endTime && cur.duration && cur.time) {
            const m = /^P(?:(\d+)D)?(?:T(?:(\d+)H)?(?:(\d+)M)?)?/.exec(cur.duration);
            if (m) {
              const days = parseInt(m[1] || "0", 10);
              const hours = parseInt(m[2] || "0", 10);
              const mins = parseInt(m[3] || "0", 10);
              if (days === 0 && (hours || mins)) {
                const [sh, sm] = cur.time.split(":").map(Number);
                let totalMin = sh * 60 + sm + hours * 60 + mins;
                const endH = Math.min(23, Math.floor(totalMin / 60));
                const endM = totalMin % 60;
                cur.endTime = `${String(endH).padStart(2, "0")}:${String(endM).padStart(2, "0")}`;
                cur.endDate = cur.date;
              }
            }
          }
          rawEvents.push(cur);
      ...
      if (key === "DURATION") cur.duration = value;
      ...
      if (key === "STATUS") cur.status = value.trim().toUpperCase();
      if (key === "ATTENDEE") {
        const partstatMatch = /PARTSTAT=([A-Z-]+)/i.exec(rawKey);
        const emailParamMatch = /EMAIL=([^;:]+)/i.exec(rawKey);
        const email = (emailParamMatch ? emailParamMatch[1] : value.replace(/^mailto:/i, "")).toLowerCase();
        if (partstatMatch && partstatMatch[1].toUpperCase() === "DECLINED" && SELF_CALENDAR_EMAILS.includes(email)) {
          cur.selfDeclined = true;
        }
      }
    }
    ...
    for (const ev of masters) {
      if (!ev.date) continue;
      if (ev.status === "CANCELLED" || ev.selfDeclined) continue;
      if (!ev.rrule) {
        const override = overrideMap[`${ev.uid}::${ev.date}`];
        if (override && (override.status === "CANCELLED" || override.selfDeclined)) continue;
      ...
      while (iterations < 500) {
        ...
        if (occDate >= windowStartISO && !ev.exdates.includes(occDate)) {
          ...
          if (override) {
            if (override.status !== "CANCELLED" && !override.selfDeclined) results.push(override);
          } else {
  ...
  var NUTRITION_TARGET_MAX = { calories: 1e4, protein: 1e3, carbs: 1500, fat: 500, fiber: 200, calcium: 5e3 };
  function sanitizeNutritionTargets(targets) {
    if (!targets) return targets;
    const cleaned = { ...targets };
    let changed = false;
    Object.keys(NUTRITION_TARGET_MAX).forEach((field) => {
      const v = cleaned[field];
      if (typeof v === "number" && v > NUTRITION_TARGET_MAX[field]) {
        cleaned[field] = null;
        changed = true;
      }
    });
    return changed ? cleaned : targets;
  }
  var PER_SERVING_MACRO_MAX = { calories: 5e3, protein: 300, carbs: 500, fat: 300, fiber: 100, calcium: 3e3 };
  function clampMacroValue(field, num) {
    if (typeof num !== "number" || isNaN(num)) return num;
    const cap = PER_SERVING_MACRO_MAX[field] || 5e3;
  ...
  function sanitizeRecipes(recipes) {
    ...
    const cleaned = recipes.map((r) => {
      let rChanged = false;
      const next = { ...r };
      Object.keys(PER_SERVING_MACRO_MAX).forEach((field) => {
        const v = next[field];
        if (typeof v === "number" && v > PER_SERVING_MACRO_MAX[field]) {
          next[field] = null;
          rChanged = true;
        }
      });
      if (rChanged) changed = true;
      return rChanged ? next : r;
    });
    return changed ? cleaned : recipes;
  }
  function sanitizeMealsExtraLogs(meals) {
    if (!meals || typeof meals !== "object") return meals;
    let changed = false;
    const cleaned = {};
    Object.keys(meals).forEach((date) => {
      const entry = meals[date];
      if (!entry || !Array.isArray(entry.extraLogs) || entry.extraLogs.length === 0) {
        cleaned[date] = entry;
        return;
      }
      let entryChanged = false;
      const nextLogs = entry.extraLogs.map((log) => {
        let logChanged = false;
        const nextLog = { ...log };
        Object.keys(PER_SERVING_MACRO_MAX).forEach((field) => {
          const v = nextLog[field];
          if (typeof v === "number" && v > PER_SERVING_MACRO_MAX[field]) {
            nextLog[field] = 0;
            logChanged = true;
          }
        });
        if (logChanged) entryChanged = true;
        return logChanged ? nextLog : log;
      });
      if (entryChanged) {
        changed = true;
        cleaned[date] = { ...entry, extraLogs: nextLogs };
      } else cleaned[date] = entry;
    });
    return changed ? cleaned : meals;
  }
  function todayDateOnly() {
    return (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  }
  var ZIIP_14_DAY_LIFT_START = "2026-09-23";
  var ZIIP_14_DAY_LIFT = [
    { t: [["The Everything Facial", 12]] },
    { t: [["Neck", 6], ["Sculpt", 6], ["Eyes", 2]] },
    { t: [["The Everything Facial", 12], ["Glow Renewal", 10]] },
    { rest: true },
    { t: [["Refresh & Depuff", 6], ["Eyes", 2], ["Instant Lift", 6]] },
    { t: [["The Everything Facial", 12], ["Glow Renewal", 10]] },
    { t: [["The Everything Facial", 12]] },
    { t: [["Instant Lift", 6], ["Eyes", 2], ["Glow Renewal", 10]] },
    { t: [["Instant Lift", 6], ["Glow Renewal", 10]] },
    { rest: true },
    { t: [["Refresh & Depuff", 6], ["Sculpt", 6], ["Eyes", 2]] },
    { t: [["The Everything Facial", 12], ["Glow Renewal", 10]] },
    { t: [["Refresh & Depuff", 6], ["Sculpt", 6], ["Eyes", 2]] },
    { t: [["Instant Lift", 6], ["Eyes", 2], ["Glow Renewal", 10]] }
  ];
  function computeZiipForDate(dateStr, ziipSettings) {
    if (dateStr >= ZIIP_14_DAY_LIFT_START) {
      const idx2 = Math.max(0, daysBetween(ZIIP_14_DAY_LIFT_START, dateStr));
      const plan2 = idx2 < ZIIP_14_DAY_LIFT.length ? ZIIP_14_DAY_LIFT[idx2] : null;
      return { ziipDayIdx: idx2, ziipToday: plan2, ziipPlanName: "14-Day Lift", ziipPlanLength: ZIIP_14_DAY_LIFT.length };
    }
    const idx = Math.max(0, daysBetween(ziipSettings.startDate, dateStr));
    const plan = idx < ZIIP_PLAN.length ? ZIIP_PLAN[idx] : null;
    return { ziipDayIdx: idx, ziipToday: plan, ziipPlanName: "Tone Reset", ziipPlanLength: ZIIP_PLAN.length };
  }
  var PILATES_ANCHOR_DATE = "2026-09-24";
  var PILATES_PRE_ROTATION_FALLBACK = "Pilates \u2014 Nell Hoess Level 4, Full Body";
  var PILATES_LEVEL4_SERIES = [
    "Pilates \u2014 Nell Hoess Level 4A, Stability focus (60 min)",
    "Pilates \u2014 Nell Hoess Level 4B, Warmup focus (43 min)",
    "Pilates \u2014 Nell Hoess Level 4C, Leg & footwork focus (60 min)",
    "Pilates \u2014 Nell Hoess Level 4D, Abdominal focus (38 min)",
    "Pilates \u2014 Nell Hoess Level 4E, Feet in straps focus (42 min)",
    "Pilates \u2014 Nell Hoess Level 4F, Lunge and Hip Disassociation (40 min)",
    "Pilates \u2014 Nell Hoess Level 4G, Long and Short Box focus (46 min)",
    "Pilates \u2014 Nell Hoess Level 4, Full Body (60 min)"
  ];
  var PILATES_LEVEL5_SERIES = [
    "Pilates \u2014 Nell Hoess Level 5A, Stability focus (45 min)",
    "Pilates \u2014 Nell Hoess Level 5B, Warmup focus (33 min)",
    "Pilates \u2014 Nell Hoess Level 5C, Leg & footwork focus (48 min)",
    "Pilates \u2014 Nell Hoess Level 5D, Abdominal focus (39 min)",
    "Pilates \u2014 Nell Hoess Level 5E, Feet in straps focus (30 min)",
    "Pilates \u2014 Nell Hoess Level 5F, Lunge and Hip Disassociation (34 min)",
    "Pilates \u2014 Nell Hoess Level 5G, Long and Short Box focus (36 min)",
    "Pilates \u2014 Nell Hoess Level 5, Full Body (64 min)"
  ];
  var PILATES_LEVEL6_SERIES = [
    "Pilates \u2014 Nell Hoess Level 6A, Stability focus (45 min)",
    "Pilates \u2014 Nell Hoess Level 6B, Warmup focus (37 min)",
    "Pilates \u2014 Nell Hoess Level 6C, Leg & footwork focus (47 min)",
    "Pilates \u2014 Nell Hoess Level 6D, Abdominal focus (39 min)",
    "Pilates \u2014 Nell Hoess Level 6E, Feet in straps focus (53 min)",
    "Pilates \u2014 Nell Hoess Level 6F, Lunge and Hip Disassociation (34 min)",
    "Pilates \u2014 Nell Hoess Level 6G, Long and Short Box focus (50 min)",
    "Pilates \u2014 Nell Hoess Level 6, Full Body (56 min)"
  ];
  var PILATES_ROTATION_SERIES = [...PILATES_LEVEL4_SERIES, ...PILATES_LEVEL5_SERIES, ...PILATES_LEVEL6_SERIES];
  var PILATES_ALL_OPTIONS = [...PILATES_ROTATION_SERIES];
  function computePilatesSession(dateStr) {
    const daysSince = daysBetween(PILATES_ANCHOR_DATE, dateStr);
    if (daysSince < 0) return PILATES_PRE_ROTATION_FALLBACK;
    const weekNum = Math.floor(daysSince / 7);
    const remainder = daysSince % 7;
    const n = weekNum * 2 + (remainder === 0 ? 0 : 1);
  ...
  var C25K_PROGRAM = [
    "C25K Week 1 \u2014 30 min: 5 min brisk walk warm-up, 20 min alternating 60s jog/90s walk, 5 min cool-down @ 7km/h",
    "C25K Week 2 \u2014 30 min: 5 min brisk walk warm-up, 20 min alternating 90s jog/60s walk, 5 min cool-down @ 7.3km/h",
    "C25K Week 3 \u2014 28 min: 5 min brisk walk warm-up, 2 rounds of (90s jog/90s walk + 3min jog/3min walk), 5 min cool-down @ 7.5km/h",
    "C25K Week 4 \u2014 31.5 min: 5 min brisk walk warm-up, jog3/walk1.5/jog5/walk2.5/jog3/walk1.5/jog5 (mins), 5 min brisk walk cool-down @ 7.7km/h",
    [
      "C25K Week 5 Day 1 \u2014 31 min: 5 min warm-up walk, jog5/walk3/jog5/walk3/jog5 (mins), 5 min cool-down walk @ 8km/h",
      "C25K Week 5 Day 2 \u2014 31 min: 5 min warm-up, 8 min jog, 5 min walk, 8 min jog, 5 min cool-down walk @ 8km/h",
      "C25K Week 5 Day 3 \u2014 30 min: 5 min brisk walk warm-up, 20 min jog, 5 min cool-down @ 8km/h"
    ],
    [
      "C25K Week 6 Day 1 \u2014 34 min: 5 min warm-up, jog5/walk3/jog8/walk3/jog5 (mins), 5 min cool-down @ 8km/h",
      "C25K Week 6 Day 2 \u2014 33 min: 5 min warm-up, 10 min jog, 3 min walk, 10 min jog, 5 min cool-down @ 8km/h",
      "C25K Week 6 Day 3 \u2014 32 min: 5 min warm-up, 22 min jog, 5 min cool-down @ 8km/h"
    ],
    "C25K Week 7 \u2014 35 min: 5 min warm-up, 25 min jog, 5 min cool-down @ 8.2km/h",
    "C25K Week 8 \u2014 38 min: 5 min warm-up, 28 min jog, 5 min cool-down @ 8.4km/h",
    "C25K Week 9 \u2014 40 min: 5 min warm-up, 30 min jog, 5 min cool-down @ 8.5km/h"
  ];
  var RUN_DAY_FOLLOWUP = ["Caroline Girvan Stretch (20 min)", "Hip stretch (15 min)", "Full body yoga"];
  function computeC25KSession(dateStr, block2StartDate) {
    const daysSince = daysBetween(block2StartDate, dateStr);
    const weekInBlock = Math.max(0, Math.floor(daysSince / 7));
    const dow = dayOfWeekISO(dateStr);
    const runDayIndex = dow === 1 ? 0 : dow === 3 ? 1 : dow === 5 ? 2 : 0;
    const weekProgram = C25K_PROGRAM[Math.min(weekInBlock, C25K_PROGRAM.length - 1)];
    const runText = Array.isArray(weekProgram) ? weekProgram[runDayIndex] : weekProgram;
    return `${runText} + ${RUN_DAY_FOLLOWUP[runDayIndex]}`;
  }
  var IRON_BLOCK1_SCHEDULE = [
    { mon: [1, 2], wed: [4], fri: [3, 5] },
    { mon: [6, 7], wed: [9], fri: [8, 10] },
    { mon: [11, 12], wed: [14], fri: [13, 15] },
    { mon: [16, 17], wed: [19], fri: [18, 20] },
    { mon: [21, 22], wed: [24], fri: [23, 25] },
    { mon: [26, 27], wed: [29], fri: [28, 30] }
  ];
  function computeIronBlock1Session(dateStr, blockStartDate) {
    const daysSince = daysBetween(blockStartDate, dateStr);
    const weekNum = Math.min(IRON_BLOCK1_SCHEDULE.length - 1, Math.max(0, Math.floor(daysSince / 7)));
    const dow = dayOfWeekISO(dateStr);
    const weekPlan = IRON_BLOCK1_SCHEDULE[weekNum];
    const dayNums = dow === 1 ? weekPlan.mon : dow === 3 ? weekPlan.wed : weekPlan.fri;
    const label = dayNums.map((n) => `Day ${n} \u2014 ${IRON_DAYS[n] ? IRON_DAYS[n].title : "Full Body"}`).join(" + ");
    return `Iron Series: ${label} (${dayNums.length * 30} min)`;
  }
  var ROUVY_LEVEL_UP = [
    ["Nice And Easy | Level Up (45 min, easy)", "Activation | Level Up (60 min, medium \u2014 Endurance)", "Fun With Fundamentals | Level Up (60 min, easy \u2014 Endurance)"],
    ["Power Endurance | Level Up (44 min, easy \u2014 Strength)", "Intro to HIIT | Level Up (43 min, easy \u2014 Threshold)", "Fat Max | Level Up (50 min, easy \u2014 Endurance)"],
    ["Sprinting | Level Up (40 min, easy \u2014 Sprinting)", "Level Up - Sweet Spot | Level Up (40 min, easy \u2014 Sweet spot/tempo)", "Against The Headwind | Level Up (55 min, medium \u2014 Threshold)"],
    ["High Cadence | Level Up (40 min, easy \u2014 Cadence)", "The Ramp Challenge (35 min, hard)", "Bridge Sprints | Level Up (45 min, easy \u2014 Sprinting)"],
    ["Oxygen Detector | Level Up (35 min, easy \u2014 VO2 Max)", "Cadence Queen/King | Level Up (30 min, easy \u2014 Cadence)", "The Long Ride | Level Up (1h 20min, medium \u2014 Endurance)"],
    ["Miracle Sprints | Level Up (37 min, easy \u2014 Sprinting)", "Fat Burner 2 | Level Up (60 min, easy \u2014 Endurance)", "The Racer | Level Up (51 min, medium \u2014 VO2 Max)"]
  ];
  function computeCyclingBlockSession(dateStr, blockStartDate) {
    const daysSince = daysBetween(blockStartDate, dateStr);
    const weekNum = Math.min(ROUVY_LEVEL_UP.length - 1, Math.max(0, Math.floor(daysSince / 7)));
    const dow = dayOfWeekISO(dateStr);
    const slotIndex = dow === 1 ? 0 : dow === 3 ? 1 : 2;
    return ROUVY_LEVEL_UP[weekNum][slotIndex];
  }
  var IRON_BLOCK2_FULLBODY_DAYS = [4, 9, 14, 19, 24, 29];
  function computeIronBlock2FullBody(dateStr, blockStartDate) {
    const daysSince = daysBetween(blockStartDate, dateStr);
    const weekNum = Math.min(IRON_BLOCK2_FULLBODY_DAYS.length - 1, Math.max(0, Math.floor(daysSince / 7)));
    const dayNum = IRON_BLOCK2_FULLBODY_DAYS[weekNum];
    const dayData = IRON_DAYS[dayNum];
    return `Iron Series Day ${dayNum} \u2014 ${dayData ? dayData.title : "Full Body"} (30 min)`;
  }
  var BLOCK_WEEKS = [6, 6, 9, 6];
  function computeFitnessForDate(dateStr, fitnessSettings, fitnessOverrides) {
    ...
    let blockIdx = BLOCK_WEEKS.length - 1;
    let weeksBefore = 0;
    for (let i = 0; i < BLOCK_WEEKS.length; i++) {
      if (weekNum <= weeksBefore + BLOCK_WEEKS[i]) {
        blockIdx = i;
        break;
      }
      weeksBefore += BLOCK_WEEKS[i];
    }
    ...
    } else if (scheduledSession === "__C25K_ROTATION__") {
      const runningBlockStartDate = addDaysISO(fitnessSettings.startDate, (BLOCK_WEEKS[0] + BLOCK_WEEKS[1]) * 7);
      scheduledSession = computeC25KSession(dateStr, runningBlockStartDate);
    } else if (scheduledSession === "__IRON_BLOCK1__") {
      scheduledSession = computeIronBlock1Session(dateStr, fitnessSettings.startDate);
    } else if (scheduledSession === "__CYCLING_BLOCK2__") {
      const cyclingBlockStartDate = addDaysISO(fitnessSettings.startDate, BLOCK_WEEKS[0] * 7);
      scheduledSession = computeCyclingBlockSession(dateStr, cyclingBlockStartDate);
    } else if (scheduledSession === "__IRON_BLOCK2_FULLBODY__") {
      const cyclingBlockStartDate = addDaysISO(fitnessSettings.startDate, BLOCK_WEEKS[0] * 7);
      scheduledSession = computeIronBlock2FullBody(dateStr, cyclingBlockStartDate);
    }
  ...
  function computeFocusStreak(today, focusId, focusDone) {
    let streak = 0;
    let d = today;
    for (let i = 0; i < 400; i++) {
      const isDone = (focusDone[d] || []).includes(focusId);
      if (isDone) {
        streak++;
      } else if (d === today) {
      } else {
  ...
  function focusHeatmapDays(today, focusId, focusDone, numDays) {
    const days = [];
    for (let i = numDays - 1; i >= 0; i--) {
      const d = addDaysISO(today, -i);
      days.push({ date: d, done: (focusDone[d] || []).includes(focusId) });
    }
    return days;
  }
  ...
  function CollapseHeader({ collapsed, onToggle, children }) {
    return /* @__PURE__ */ React.createElement(
      "button",
      {
        onClick: onToggle,
      ...
      /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft, transform: collapsed ? "rotate(-90deg)" : "none", display: "inline-block", transition: "transform 0.15s" } }, "\u25BE"),
      children
    );
  }
  ...
  function RecipeAutocomplete({ recipes, onSelectRecipe, onSelectCustom, placeholder, style }) {
    const [query, setQuery] = useState("");
    const [open, setOpen] = useState(false);
    const sorted = [...recipes].sort((a, b) => a.name.localeCompare(b.name));
    const q = query.trim().toLowerCase();
    const matches = q ? sorted.filter((r) => r.name.toLowerCase().includes(q)) : sorted;
    const commitCustom = () => {
      if (onSelectCustom && query.trim()) {
        onSelectCustom(query.trim());
        setQuery("");
        setOpen(false);
      }
    };
    return /* @__PURE__ */ React.createElement("div", { style: { position: "relative", flex: 1, minWidth: 160, ...style } }, /* @__PURE__ */ React.createElement(
      Input,
      {
        placeholder: placeholder || "Type to search recipes\u2026",
        value: query,
        onChange: (e) => {
        ...
        onKeyDown: (e) => {
          if (e.key === "Enter" && matches.length === 0) commitCustom();
        },
        style: { width: "100%" }
      }
    ), open && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", zIndex: 5, left: 0, right: 0, border: `1px solid ${COLORS.line}`, borderRadius: 8, background: "#fff", marginTop: 2, maxHeight: 180, overflowY: "auto", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" } }, matches.length === 0 ? query.trim() ? onSelectCustom ? /* @__PURE__ */ React.createElement("button", { onMouseDown: (e) => e.preventDefault(), onClick: commitCustom, style: { display: "block", width: "100%", textAlign: "left", background: "none", border: "none", padding: "8px 10px", fontSize: 13, cursor: "pointer", color: COLORS.green } }, '+ Add "', query.trim(), '" as custom item') : /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 10px", fontSize: 12.5, color: COLORS.inkSoft } }, "No matching recipes.") : /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 10px", fontSize: 12.5, color: COLORS.inkSoft } }, "No recipes yet.") : matches.slice(0, 40).map((r) => /* @__PURE__ */ React.createElement(
      "button",
      {
        key: r.id,
        ...
        style: { display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", textAlign: "left", background: "none", border: "none", padding: "8px 10px", fontSize: 13.5, cursor: "pointer", color: COLORS.ink, borderBottom: `1px solid ${COLORS.line}` }
      },
      /* @__PURE__ */ React.createElement("span", null, r.name),
      /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10.5, fontWeight: 700, color: COLORS.inkSoft, marginLeft: 8, flexShrink: 0 } }, r.type === "item" ? "ITEM" : "RECIPE")
    ))));
  }
  function BlockPicker({ value, onSelect, taskOptions, placeholder, colorHex }) {
    const [query, setQuery] = useState("");
    const [open, setOpen] = useState(false);
    const [expandedGroups, setExpandedGroups] = useState({});
    const displayLabel = value ? value.category === "other" ? value.label : (timeBlockCategoryOf(value.category) || {}).label || "" : "";
    const q = query.trim().toLowerCase();
    const catMatches = TIME_BLOCK_CATEGORIES.filter((c) => !q || c.label.toLowerCase().includes(q));
    const taskMatches = (taskOptions || []).filter((t) => !q || t.text.toLowerCase().includes(q));
    const commitCustom = () => {
      if (query.trim()) {
        onSelect({ category: "other", label: query.trim() });
        setQuery("");
        setOpen(false);
      }
    };
    const pick = (label) => {
      onSelect({ category: "other", label });
      setQuery("");
      setOpen(false);
    };
    const groupOrder = ["To-do list", ...POOL_CATEGORIES.map((c) => c.label), "Uncategorised", "Recurring tasks"];
    const groups = groupOrder.map((label) => ({ label, items: (taskOptions || []).filter((t) => t.groupLabel === label) })).filter((g) => g.items.length > 0);
    return /* @__PURE__ */ React.createElement("div", { style: { position: "relative", flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement(
      Input,
      {
        ...
        onBlur: () => setTimeout(() => setOpen(false), 150),
        onKeyDown: (e) => {
          if (e.key === "Enter" && catMatches.length === 0 && taskMatches.length === 0) commitCustom();
        },
        style: { width: "100%", fontSize: 11, padding: "4px 4px", background: "#fff", borderColor: COLORS.line }
      }
    ), open && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", zIndex: 6, left: 0, right: 0, border: `1px solid ${COLORS.line}`, borderRadius: 8, background: "#fff", marginTop: 2, maxHeight: 260, overflowY: "auto", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" } }, !q ? /* @__PURE__ */ React.createElement(React.Fragment, null, groups.map((g) => {
      const isOpen = !!expandedGroups[g.label];
      return /* @__PURE__ */ React.createElement("div", { key: g.label }, /* @__PURE__ */ React.createElement(
        "button",
        {
          onMouseDown: (e) => e.preventDefault(),
          onClick: () => setExpandedGroups({ ...expandedGroups, [g.label]: !isOpen }),
          style: { display: "flex", justifyContent: "space-between", width: "100%", textAlign: "left", background: COLORS.bg, border: "none", padding: "6px 10px", fontSize: 11, fontWeight: 700, color: COLORS.inkSoft, cursor: "pointer" }
        },
        /* @__PURE__ */ React.createElement("span", null, isOpen ? "\u25BE" : "\u25B8", " ", g.label),
        /* @__PURE__ */ React.createElement("span", null, g.items.length)
      ), isOpen && g.items.map((t) => /* @__PURE__ */ React.createElement(
        "button",
        {
          key: t.id,
          onMouseDown: (e) => e.preventDefault(),
          onClick: () => pick(t.text),
          style: { display: "block", width: "100%", textAlign: "left", background: "none", border: "none", padding: "7px 10px 7px 20px", fontSize: 13, cursor: "pointer", color: COLORS.ink, borderBottom: `1px solid ${COLORS.line}` }
        },
        t.text
      )));
    }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: COLORS.inkSoft, fontWeight: 700, padding: "6px 10px", background: COLORS.bg } }, "Categories"), TIME_BLOCK_CATEGORIES.map((c) => /* @__PURE__ */ React.createElement(
      "button",
      {
        key: c.key,
        onMouseDown: (e) => e.preventDefault(),
        onClick: () => {
          onSelect({ category: c.key, label: "" });
          setQuery("");
          setOpen(false);
        },
        style: { display: "flex", alignItems: "center", gap: 8, width: "100%", textAlign: "left", background: "none", border: "none", padding: "7px 10px", fontSize: 13, cursor: "pointer", color: COLORS.ink, borderBottom: `1px solid ${COLORS.line}` }
      },
      /* @__PURE__ */ React.createElement("span", { style: { width: 9, height: 9, borderRadius: 5, background: c.color, flexShrink: 0 } }),
      /* @__PURE__ */ React.createElement("span", null, c.label)
    ))) : /* @__PURE__ */ React.createElement(React.Fragment, null, taskMatches.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: COLORS.inkSoft, fontWeight: 700, padding: "4px 10px", background: COLORS.bg } }, "From your lists"), taskMatches.map((t) => /* @__PURE__ */ React.createElement(
      "button",
      {
        key: t.id,
        onMouseDown: (e) => e.preventDefault(),
        onClick: () => pick(t.text),
        style: { display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", textAlign: "left", background: "none", border: "none", padding: "7px 10px", fontSize: 13, cursor: "pointer", color: COLORS.ink, borderBottom: `1px solid ${COLORS.line}` }
      },
      /* @__PURE__ */ React.createElement("span", null, t.text),
      /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9.5, fontWeight: 700, color: COLORS.inkSoft, marginLeft: 8, flexShrink: 0 } }, t.groupLabel)
    ))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: COLORS.inkSoft, fontWeight: 700, padding: "4px 10px", background: COLORS.bg } }, "Categories"), catMatches.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 10px", fontSize: 12.5, color: COLORS.inkSoft } }, 'No matches \u2014 press Enter to use "', query.trim(), '" as custom.') : catMatches.map((c) => /* @__PURE__ */ React.createElement(
      "button",
      {
        key: c.key,
        onMouseDown: (e) => e.preventDefault(),
  ...
  function App() {
    ...
    const [ledMaskMissed, setLedMaskMissed] = useState({});
    const [skinPurchases, setSkinPurchases] = useState([]);
    const [timeScheduleSettings, setTimeScheduleSettings] = useState({ dayStart: "06:00", dayEnd: "22:00" });
    ...
    const [recurringMissed, setRecurringMissed] = useState({});
    const [apptAttended, setApptAttended] = useState({});
    const [skinDayStatus, setSkinDayStatus] = useState({});
    ...
    const syncIcloudCalendar = async (urlOverride) => {
      ...
      const fetchUrl = corsProxyUrl ? `${corsProxyUrl}${corsProxyUrl.includes("?") ? "&" : "?"}url=${encodeURIComponent(httpsUrl)}&${cacheBust}` : `${httpsUrl}${httpsUrl.includes("?") ? "&" : "?"}${cacheBust}`;
      ...
      try {
        ...
        const items = events.filter((ev) => ev.date && ev.date >= todayStr && ev.date <= maxDateStr).map((ev) => {
          const hasEnd = ev.endTime && (!ev.endDate || ev.endDate === ev.date);
          const timePart = ev.time ? hasEnd ? `${ev.time}-${ev.endTime}` : ev.time : null;
          const text2 = timePart ? `${timePart} \u2014 ${ev.summary || "(untitled)"}` : ev.summary || "(untitled)";
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        try {
          ...
          const focusItemsR = await readOrMissing("focus-items");
          const focusDoneR = await readOrMissing("focus-done");
          const focusMissedR = await readOrMissing("focus-missed");
          const recurringMissedR = await readOrMissing("recurring-missed");
          const apptAttendedR = await readOrMissing("appt-attended");
          const skinDayStatusR = await readOrMissing("skin-day-status");
          ...
          const skinPurchasesR = await readOrMissing("skin-purchases");
          const timeScheduleSettingsR = await readOrMissing("time-schedule-settings");
          const morningTemplateR = await readOrMissing("morning-template");
          const eveningTemplateR = await readOrMissing("evening-template");
          const timeSchedulesR = await readOrMissing("time-schedules");
          ...
          setSkinPurchases(skinPurchasesR.found ? skinPurchasesR.value : []);
          setTimeScheduleSettings(timeScheduleSettingsR.found ? timeScheduleSettingsR.value : { dayStart: "06:00", dayEnd: "22:00" });
          setMorningTemplate(morningTemplateR.found ? morningTemplateR.value : []);
          setEveningTemplate(eveningTemplateR.found ? eveningTemplateR.value : []);
          setTimeSchedules(timeSchedulesR.found ? timeSchedulesR.value : {});
          ...
          setFocusItems(focusItemsR.found ? focusItemsR.value : []);
          setFocusDone(focusDoneR.found ? focusDoneR.value : {});
          setFocusMissed(focusMissedR.found ? focusMissedR.value : {});
          setRecurringMissed(recurringMissedR.found ? recurringMissedR.value : {});
          setApptAttended(apptAttendedR.found ? apptAttendedR.value : {});
          setSkinDayStatus(skinDayStatusR.found ? skinDayStatusR.value : {});
          ...
          setMeals(mealsR.found ? sanitizeMealsExtraLogs(mealsR.value) : {});
          setDismissedIds(dismissedIdsV);
          setRecipes(recipesR.found ? sanitizeRecipes(recipesR.value) : RECIPE_SEED);
          ...
          if (nutritionTargetsR.found && nutritionTargetsR.value) setNutritionTargets(sanitizeNutritionTargets(nutritionTargetsR.value));
    ...
    usePersist(loaded, persist, "skin-purchases", skinPurchases);
    usePersist(loaded, persist, "time-schedule-settings", timeScheduleSettings);
    usePersist(loaded, persist, "morning-template", morningTemplate);
    usePersist(loaded, persist, "evening-template", eveningTemplate);
    usePersist(loaded, persist, "time-schedules", timeSchedules);
    ...
    usePersist(loaded, persist, "focus-items", focusItems);
    usePersist(loaded, persist, "focus-done", focusDone);
    usePersist(loaded, persist, "focus-missed", focusMissed);
    usePersist(loaded, persist, "recurring-missed", recurringMissed);
    usePersist(loaded, persist, "appt-attended", apptAttended);
    usePersist(loaded, persist, "skin-day-status", skinDayStatus);
    ...
    const { ziipDayIdx, ziipToday, ziipPlanName, ziipPlanLength } = computeZiipForDate(today, ziipSettings);
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { minHeight: "100vh", background: COLORS.bg, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", paddingBottom: 40 } }, /* @__PURE__ */ React.createElement("div", { style: { maxWidth: 640, margin: "0 auto", padding: "20px 16px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: "Georgia, serif", fontSize: 26, color: COLORS.ink } }, "Kirsty"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700, color: COLORS.ink } }, fmtDate(today)), /* @__PURE__ */ React.createElement(
      ...
      TodayTab,
      {
        ...
        setSkinDayStatus,
        setWeeklyPool,
        timeScheduleSettings,
        setTimeScheduleSettings,
        morningTemplate,
        setMorningTemplate,
        eveningTemplate,
        setEveningTemplate,
  ...
  function TodayTab({ today, viewDate, setViewDate, fitnessSettings, fitnessOverrides, fitnessLogs, setFitnessLogs, ziipSettings, skinPlan, ledMaskDone, setLedMaskDone, todos, setTodos, weeklyPool, todayPicks, setTodayPicks, poolDone, setPoolDone, dailyOrder, setDailyOrder, recurringTasks, recurringDone, setRecurringDone, focusItems, setFocusItems, focusDone, setFocusDone, focusMissed, setFocusMissed, recurringMissed, setRecurringMissed, apptAttended, setApptAttended, skinDayStatus, setSkinDayStatus, setWeeklyPool, timeScheduleSettings, setTimeScheduleSettings, morningTemplate, setMorningTemplate, eveningTemplate, setEveningTemplate, timeSchedules, setTimeSchedules, setTab }) {
    ...
    const todayFitnessLog = (fitnessLogs || []).find((l) => l.date === viewDate);
    const fitnessStatus = todayFitnessLog ? todayFitnessLog.status : null;
    const setFitnessGlanceStatus = (status) => {
      const others = (fitnessLogs || []).filter((l) => l.date !== viewDate);
      if (fitnessStatus === status) {
        setFitnessLogs(others);
      } else {
        setFitnessLogs([...others, { date: viewDate, session: regularSession, note: "", status }].sort((a, b) => a.date.localeCompare(b.date)));
      }
    };
    const skinStatusToday = skinDayStatus[viewDate] || null;
    const setSkinStatus = (status) => {
      if (skinStatusToday === status) {
        const next = { ...skinDayStatus };
        delete next[viewDate];
        setSkinDayStatus(next);
      } else {
        setSkinDayStatus({ ...skinDayStatus, [viewDate]: status });
      }
    };
    const appts = (todos || []).filter((t) => t.date === viewDate && (t.source === "calendar" || t.source === "calendar-live" || t.source === "retreat")).sort((a, b) => apptSortKey(a.text).localeCompare(apptSortKey(b.text)));
    ...
    const promoteToPool = (t) => {
      const steps = (t.steps || []).map((s) => ({ ...s, id: "pstep-" + Date.now() + "-" + Math.random().toString(36).slice(2, 5) }));
      setWeeklyPool([...weeklyPool, { id: "pool-" + Date.now(), text: t.text, steps }]);
      removeQuickTask(t.id);
    };
    ...
    const toggleStepsExpanded = (todoId) => setExpandedSteps({ ...expandedSteps, [todoId]: !expandedSteps[todoId] });
    const addTodoStep = (todoId, text) => {
      if (!text.trim()) return;
      setTodos((todos || []).map((t) => t.id === todoId ? { ...t, steps: [...t.steps || [], { id: "step-" + Date.now(), text: text.trim(), done: false }] } : t));
      setExpandedSteps({ ...expandedSteps, [todoId]: true });
    };
    const toggleTodoStepDone = (todoId, stepId) => {
      setTodos((todos || []).map((t) => t.id === todoId ? { ...t, steps: (t.steps || []).map((s) => s.id === stepId ? { ...s, done: !s.done } : s) } : t));
    };
    const removeTodoStep = (todoId, stepId) => {
      setTodos((todos || []).map((t) => t.id === todoId ? { ...t, steps: (t.steps || []).filter((s) => s.id !== stepId) } : t));
    };
    const moveTodoStep = (todoId, stepId, direction) => {
      setTodos((todos || []).map((t) => {
        if (t.id !== todoId) return t;
        const steps = t.steps || [];
        const idx = steps.findIndex((s) => s.id === stepId);
        const swapIdx = idx + direction;
        if (idx === -1 || swapIdx < 0 || swapIdx >= steps.length) return t;
        const next = [...steps];
    ...
    const startEditTodoStep = (s) => {
      setEditingStepId(s.id);
      setEditStepText(s.text);
    };
    const saveEditTodoStep = (todoId, stepId) => {
      if (editStepText.trim()) {
        setTodos((todos || []).map((t) => t.id === todoId ? { ...t, steps: (t.steps || []).map((s) => s.id === stepId ? { ...s, text: editStepText.trim() } : s) } : t));
      }
      setEditingStepId(null);
    };
    const [expandedFocusSteps, setExpandedFocusSteps] = useState({});
    const toggleFocusStepsExpanded = (focusId) => setExpandedFocusSteps({ ...expandedFocusSteps, [focusId]: !expandedFocusSteps[focusId] });
    const toggleFocusStepDone = (focusId, stepId) => {
      setFocusItems((focusItems || []).map((f) => f.id === focusId ? { ...f, steps: (f.steps || []).map((s) => s.id === stepId ? { ...s, done: !s.done } : s) } : f));
    };
    const naturalItems = [
    ...
    const isItemDone = (t) => t.kind === "pool" ? doneIds.includes(t.id) : !!t.done;
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Card, { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => shiftDay(-1), style: { background: "none", border: "none", color: COLORS.green, fontSize: 18, cursor: "pointer", padding: "4px 8px" } }, "\u2039"), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14.5, fontWeight: 700, color: COLORS.ink } }, isToday ? "Today" : fmtDate(viewDate)), !isToday && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft } }, fmtDate(viewDate)), !isToday && /* @__PURE__ */ React.createElement("button", { onClick: () => setViewDate(today), style: { background: "none", border: "none", color: COLORS.clay, fontSize: 11, cursor: "pointer", padding: 0 } }, "back to today")), /* @__PURE__ */ React.createElement("button", { onClick: () => shiftDay(1), style: { background: "none", border: "none", color: COLORS.green, fontSize: 18, cursor: "pointer", padding: "4px 8px" } }, "\u203A")), focusItems.length > 0 && /* @__PURE__ */ React.createElement(Card, { style: { background: COLORS.claySoft, borderColor: COLORS.clay } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10.5, fontWeight: 700, color: COLORS.clay, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 6 } }, "Focus"), focusItems.map((f) => {
      const doneIds2 = focusDone[viewDate] || [];
      const missedIds = focusMissed[viewDate] || [];
      const isDone = doneIds2.includes(f.id);
      const isMissed = missedIds.includes(f.id);
      const toggle = () => {
        const next = isDone ? doneIds2.filter((id) => id !== f.id) : [...doneIds2, f.id];
        setFocusDone({ ...focusDone, [viewDate]: next });
        if (!isDone && isMissed) setFocusMissed({ ...focusMissed, [viewDate]: missedIds.filter((id) => id !== f.id) });
      };
      const toggleMissed = () => {
        const next = isMissed ? missedIds.filter((id) => id !== f.id) : [...missedIds, f.id];
        setFocusMissed({ ...focusMissed, [viewDate]: next });
        if (!isMissed && isDone) setFocusDone({ ...focusDone, [viewDate]: doneIds2.filter((id) => id !== f.id) });
      };
      const streak = computeFocusStreak(today, f.id, focusDone);
      const steps = f.steps || [];
      const stepsExpanded = expandedFocusSteps[f.id];
      return /* @__PURE__ */ React.createElement("div", { key: f.id, style: { padding: "3px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: isDone, onChange: toggle, style: { accentColor: COLORS.clay, width: 16, height: 16 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 14, color: isMissed ? COLORS.danger : COLORS.ink, textDecoration: isDone ? "line-through" : "none", opacity: isDone ? 0.6 : 1 } }, f.text), streak > 1 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.clay, fontWeight: 700 } }, "\u{1F525} ", streak), /* @__PURE__ */ React.createElement(
        "button",
        {
          onClick: toggleMissed,
          title: isMissed ? "Marked not completed \u2014 tap to clear" : "Mark as not completed",
          style: { background: "none", border: "none", cursor: "pointer", color: isMissed ? COLORS.danger : COLORS.inkSoft, fontSize: isMissed ? 20 : 16, fontWeight: 900, padding: "0 2px", lineHeight: 1 }
        },
        "\u2715"
      )), steps.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginLeft: 24 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => toggleFocusStepsExpanded(f.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11.5, padding: "2px 0" } }, stepsExpanded ? "\u25BE" : "\u25B8", " steps (", steps.filter((s) => s.done).length, "/", steps.length, ")"), stepsExpanded && /* @__PURE__ */ React.createElement("div", { style: { paddingLeft: 4 } }, steps.map((s) => /* @__PURE__ */ React.createElement("div", { key: s.id, style: { display: "flex", alignItems: "center", gap: 6, padding: "2px 0" } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: s.done, onChange: () => toggleFocusStepDone(f.id, s.id), style: { accentColor: COLORS.clay, width: 14, height: 14, flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 13, color: s.done ? COLORS.inkSoft : COLORS.ink, textDecoration: s.done ? "line-through" : "none" } }, s.text))))));
    })), /* @__PURE__ */ React.createElement(Card, { style: (() => {
      const attendedIds = apptAttended[viewDate] || [];
      const allAttended = appts.length > 0 && appts.every((t) => attendedIds.includes(t.id));
      return allAttended ? { borderColor: COLORS.green, background: "#A9D6A9" } : {};
    })() }, /* @__PURE__ */ React.createElement(SectionTitle, null, "Appointments & events"), appts.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.inkSoft } }, "Nothing on the calendar.") : appts.map((t) => {
      const attendedIds = apptAttended[viewDate] || [];
      const attended = attendedIds.includes(t.id);
      const toggleAttended = () => {
        const next = attended ? attendedIds.filter((id) => id !== t.id) : [...attendedIds, t.id];
        setApptAttended({ ...apptAttended, [viewDate]: next });
      };
      return /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "5px 6px", borderRadius: 6, background: attended ? "#A9D6A9" : "transparent" } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: attended, onChange: toggleAttended, style: { accentColor: COLORS.green, width: 16, height: 16, flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 14, color: COLORS.ink, textDecoration: attended ? "line-through" : "none", opacity: attended ? 0.75 : 1 } }, t.text));
    })), /* @__PURE__ */ React.createElement(Card, { style: { borderColor: fitnessStatus === "done" ? COLORS.green : fitnessStatus === "missed" ? COLORS.danger : COLORS.gold, background: fitnessStatus === "done" ? "#A9D6A9" : fitnessStatus === "missed" ? "#F0A99E" : isRetreatDay ? COLORS.claySoft : COLORS.card } }, retreat ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Pill, { tone: "clay" }, "RETREAT \u2014 ", retreat.theme), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 14, lineHeight: 1.6, color: COLORS.ink } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("b", null, "Yoga:"), " ", retreat.yoga), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("b", null, "Training:"), " ", retreat.training), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("b", null, "Treatment:"), " ", retreat.treatment), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, fontStyle: "italic", color: COLORS.clay } }, retreat.skinNote))) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement(Pill, null, "Week ", weekNum, " \u2014 ", block.name), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 15, color: COLORS.ink } }, regularSession), isOverridden && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, fontSize: 11.5, color: COLORS.clay } }, "Swapped from the regular schedule \u2014 change it back anytime in Fitness.")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexShrink: 0 } }, /* @__PURE__ */ React.createElement(
      "button",
      {
        onClick: () => setFitnessGlanceStatus("done"),
        title: fitnessStatus === "done" ? "Marked completed \u2014 tap to clear" : "Mark completed",
        style: { background: "none", border: "none", cursor: "pointer", color: fitnessStatus === "done" ? COLORS.green : COLORS.inkSoft, fontSize: fitnessStatus === "done" ? 22 : 18, fontWeight: 900, padding: "0 2px", lineHeight: 1 }
      },
      "\u2713"
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
        onClick: () => setFitnessGlanceStatus("missed"),
        title: fitnessStatus === "missed" ? "Marked not completed \u2014 tap to clear" : "Mark not completed",
        style: { background: "none", border: "none", cursor: "pointer", color: fitnessStatus === "missed" ? COLORS.danger : COLORS.inkSoft, fontSize: fitnessStatus === "missed" ? 22 : 18, fontWeight: 900, padding: "0 2px", lineHeight: 1 }
      },
      "\u2715"
    ))))), /* @__PURE__ */ React.createElement(Card, { style: { borderColor: skinStatusToday === "done" ? COLORS.green : skinStatusToday === "missed" ? COLORS.danger : COLORS.line, background: skinStatusToday === "done" ? "#A9D6A9" : skinStatusToday === "missed" ? "#F0A99E" : COLORS.card } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement(SectionTitle, null, "Skin")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexShrink: 0 } }, /* @__PURE__ */ React.createElement(
      "button",
      {
        onClick: () => setSkinStatus("done"),
        title: skinStatusToday === "done" ? "Marked completed \u2014 tap to clear" : "Mark completed",
        style: { background: "none", border: "none", cursor: "pointer", color: skinStatusToday === "done" ? COLORS.green : COLORS.inkSoft, fontSize: skinStatusToday === "done" ? 22 : 18, fontWeight: 900, padding: "0 2px", lineHeight: 1 }
      },
      "\u2713"
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
        onClick: () => setSkinStatus("missed"),
        title: skinStatusToday === "missed" ? "Marked not completed \u2014 tap to clear" : "Mark not completed",
        style: { background: "none", border: "none", cursor: "pointer", color: skinStatusToday === "missed" ? COLORS.danger : COLORS.inkSoft, fontSize: skinStatusToday === "missed" ? 22 : 18, fontWeight: 900, padding: "0 2px", lineHeight: 1 }
      },
      "\u2715"
    ))), ziipToday && !ziipToday.rest ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: COLORS.ink } }, "ZIIP Day ", ziipDayIdx + 1, ": ", ziipToday.t.map(([n, m]) => `${n} (${m}min)`).join(" \xB7 ")) : ziipToday === null ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: COLORS.inkSoft } }, "ZIIP: outside the current 30-day plan.") : /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: COLORS.inkSoft } }, "ZIIP: rest day"), isNonZiipDay && !isRetreatDay && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, paddingTop: 8, borderTop: `1px solid ${COLORS.line}`, display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: ledMaskDoneToday, onChange: toggleLedMask, style: { accentColor: COLORS.clay, width: 16, height: 16 } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13.5, color: COLORS.ink } }, "LED mask")), !isRetreatDay && (hasExfoliatingCleanseAm || dow === 4) && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, paddingTop: 8, borderTop: `1px solid ${COLORS.line}` } }, dow === 4 && /* @__PURE__ */ React.createElement(Pill, { tone: "clay" }, "Exfoliation night"), hasExfoliatingCleanseAm && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.ink, marginTop: 4 } }, "AM: Exfoliating cleanse day"), dow === 4 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.ink, marginTop: 4 } }, "PM: Exfoliation night")), isToday && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setTab("skin"), style: { background: "none", border: "none", color: COLORS.green, fontSize: 13, fontWeight: 600, cursor: "pointer", padding: 0 } }, "Open full skin checklist \u2192"))), dueRecurring.length > 0 && /* @__PURE__ */ React.createElement(Card, { style: dueRecurring.every((t) => recurringDoneIds.includes(t.id)) ? { borderColor: COLORS.green, background: "#A9D6A9" } : {} }, /* @__PURE__ */ React.createElement(SectionTitle, null, "Recurring tasks"), dueRecurring.map((t) => {
      const done = recurringDoneIds.includes(t.id);
      const missedIds = recurringMissed[viewDate] || [];
      const missed = missedIds.includes(t.id);
      const toggleRecurringMissed = () => {
        const next = missed ? missedIds.filter((id) => id !== t.id) : [...missedIds, t.id];
        setRecurringMissed({ ...recurringMissed, [viewDate]: next });
        if (!missed && done) toggleRecurringDone(t.id);
      };
      const toggleDoneWithClear = () => {
        toggleRecurringDone(t.id);
        if (!done && missed) setRecurringMissed({ ...recurringMissed, [viewDate]: missedIds.filter((id) => id !== t.id) });
      };
      return /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "5px 6px", borderRadius: 6, background: done ? "#A9D6A9" : missed ? "#F0A99E" : "transparent" } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: done, onChange: toggleDoneWithClear, style: { accentColor: COLORS.clay, width: 16, height: 16, flexShrink: 0 } }), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, fontSize: 14, color: missed ? COLORS.danger : done ? COLORS.inkSoft : COLORS.ink, textDecoration: done ? "line-through" : "none" } }, t.text), /* @__PURE__ */ React.createElement(
        "button",
        {
          onClick: toggleRecurringMissed,
          title: missed ? "Marked not completed \u2014 tap to clear" : "Mark as not completed",
          style: { background: "none", border: "none", cursor: "pointer", color: missed ? COLORS.danger : COLORS.inkSoft, fontSize: missed ? 20 : 16, fontWeight: 900, padding: "0 2px", lineHeight: 1, flexShrink: 0 }
        },
        "\u2715"
      ));
    })), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, isToday ? "Today's to-do list" : `To-do \u2014 ${fmtDate(viewDate)}`), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: COLORS.ink, marginBottom: 10, paddingBottom: 8, borderBottom: `1px solid ${COLORS.line}` } }, dailyStats.total > 0 ? `${dailyStats.done} of ${dailyStats.total} tasks done` : "No tasks tracked today", taskStreak > 1 && /* @__PURE__ */ React.createElement("span", { style: { marginLeft: 8, color: COLORS.clay, fontWeight: 600 } }, "\u{1F525} ", taskStreak, "-day streak")), orderedItems.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.inkSoft, marginBottom: 8 } }, "Nothing on the list for this day yet.") : /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 8 } }, orderedItems.map((t, i) => {
      const done = t.kind === "pool" ? doneIds.includes(t.id) : !!t.done;
      const toggle = t.kind === "pool" ? () => togglePoolDoneToday(t.id) : () => toggleTodoDone(t.id);
      const removeAction = t.kind === "pool" ? () => unpickToday(t.id) : () => removeQuickTask(t.id);
      const removeLabel = t.kind === "pool" ? "remove" : "\u2715";
      const removeColor = t.kind === "pool" ? COLORS.inkSoft : COLORS.danger;
      const editing = t.kind === "todo" && editingTodoId === t.id;
      const stepsExpanded = expandedSteps[t.id];
      const steps = t.steps || [];
      return /* @__PURE__ */ React.createElement("div", { key: t.id, style: { padding: "5px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: editing ? "flex-start" : "center", gap: 6, flexWrap: editing ? "wrap" : "nowrap" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", flexShrink: 0 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => moveItem(t.id, -1), disabled: i === 0, style: { background: "none", border: "none", color: i === 0 ? COLORS.line : COLORS.inkSoft, cursor: i === 0 ? "default" : "pointer", fontSize: 12, lineHeight: 1, padding: "1px 4px" } }, "\u25B2"), /* @__PURE__ */ React.createElement("button", { onClick: () => moveItem(t.id, 1), disabled: i === orderedItems.length - 1, style: { background: "none", border: "none", color: i === orderedItems.length - 1 ? COLORS.line : COLORS.inkSoft, cursor: i === orderedItems.length - 1 ? "default" : "pointer", fontSize: 12, lineHeight: 1, padding: "1px 4px" } }, "\u25BC")), /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: done, onChange: toggle, style: { accentColor: COLORS.clay, width: 16, height: 16, flexShrink: 0, marginTop: editing ? 6 : 0 } }), editing ? /* @__PURE__ */ React.createElement(
        "textarea",
        {
          value: editTodoText,
          ...
          autoFocus: true,
          rows: 2,
          style: { flex: "1 1 100%", fontSize: 14, fontFamily: "inherit", padding: "6px 8px", borderRadius: 6, border: `1px solid ${COLORS.line}`, resize: "vertical", boxSizing: "border-box" }
        }
      ) : /* @__PURE__ */ React.createElement("div", { style: { flex: 1, fontSize: 14, color: done ? COLORS.inkSoft : COLORS.ink, textDecoration: done ? "line-through" : "none" } }, t.text), t.kind === "todo" && (editing ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => saveEditTodo(t.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12, fontWeight: 700 } }, "save"), /* @__PURE__ */ React.createElement("button", { onClick: () => setEditingTodoId(null), style: { background: "none", border: "none", color: COLORS.inkSoft, cursor: "pointer", fontSize: 12 } }, "cancel")) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => promoteToPool(t), title: "Move to the weekly task pool", style: { background: "none", border: "none", color: COLORS.slate, cursor: "pointer", fontSize: 11.5 } }, "\u2192 Pool"), /* @__PURE__ */ React.createElement("button", { onClick: () => startEditTodo(t), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12 } }, "edit"))), removeAction && !editing && /* @__PURE__ */ React.createElement("button", { onClick: removeAction, title: t.kind === "pool" ? "Remove from today's list (stays in the pool)" : void 0, style: { background: "none", border: "none", color: removeColor, cursor: "pointer", fontSize: t.kind === "pool" ? 11 : 12 } }, removeLabel)), t.kind === "todo" && !editing && /* @__PURE__ */ React.createElement("div", { style: { marginLeft: 40 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => toggleStepsExpanded(t.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11.5, padding: "2px 0" } }, steps.length > 0 ? `${stepsExpanded ? "\u25BE" : "\u25B8"} steps (${steps.filter((s) => s.done).length}/${steps.length})` : `${stepsExpanded ? "\u25BE" : "\u25B8"} add steps`), stepsExpanded && /* @__PURE__ */ React.createElement("div", { style: { paddingLeft: 4 } }, steps.map((s, si) => {
        const stepEditing = editingStepId === s.id;
        return /* @__PURE__ */ React.createElement("div", { key: s.id, style: { display: "flex", alignItems: "center", gap: 6, padding: "3px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => moveTodoStep(t.id, s.id, -1), disabled: si === 0, style: { background: "none", border: "none", color: si === 0 ? COLORS.line : COLORS.inkSoft, cursor: si === 0 ? "default" : "pointer", fontSize: 10, lineHeight: 1, padding: "0 3px" } }, "\u25B2"), /* @__PURE__ */ React.createElement("button", { onClick: () => moveTodoStep(t.id, s.id, 1), disabled: si === steps.length - 1, style: { background: "none", border: "none", color: si === steps.length - 1 ? COLORS.line : COLORS.inkSoft, cursor: si === steps.length - 1 ? "default" : "pointer", fontSize: 10, lineHeight: 1, padding: "0 3px" } }, "\u25BC")), /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: s.done, onChange: () => toggleTodoStepDone(t.id, s.id), style: { accentColor: COLORS.clay, width: 14, height: 14, flexShrink: 0 } }), stepEditing ? /* @__PURE__ */ React.createElement(
          Input,
          {
            value: editStepText,
            onChange: (e) => setEditStepText(e.target.value),
            onKeyDown: (e) => {
              if (e.key === "Enter") saveEditTodoStep(t.id, s.id);
              if (e.key === "Escape") setEditingStepId(null);
            },
            autoFocus: true,
            style: { flex: 1, fontSize: 12.5 }
          }
        ) : /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 13, color: s.done ? COLORS.inkSoft : COLORS.ink, textDecoration: s.done ? "line-through" : "none" } }, s.text), stepEditing ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => saveEditTodoStep(t.id, s.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11, fontWeight: 700 } }, "save"), /* @__PURE__ */ React.createElement("button", { onClick: () => setEditingStepId(null), style: { background: "none", border: "none", color: COLORS.inkSoft, cursor: "pointer", fontSize: 11 } }, "cancel")) : /* @__PURE__ */ React.createElement("button", { onClick: () => startEditTodoStep(s), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11 } }, "edit"), /* @__PURE__ */ React.createElement("button", { onClick: () => removeTodoStep(t.id, s.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 11 } }, "\u2715"));
      }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 4 } }, /* @__PURE__ */ React.createElement(
        Input,
        {
          placeholder: "Add a step",
          value: newStepText[t.id] || "",
          onChange: (e) => setNewStepText({ ...newStepText, [t.id]: e.target.value }),
          onKeyDown: (e) => {
            if (e.key === "Enter") {
              addTodoStep(t.id, newStepText[t.id] || "");
              setNewStepText({ ...newStepText, [t.id]: "" });
            }
          },
          style: { flex: 1, fontSize: 12.5 }
        }
        ...
        "button",
        {
          ...
          style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12, fontWeight: 600 }
        },
        "add"
      )))));
  ...
  function WeekTab({ today, fitnessSettings, fitnessOverrides, fitnessLogs, ziipSettings, skinPlan, ledMaskDone, todos, meals, weeklyPool, todayPicks, poolDone, recurringTasks, recurringDone, weeklyReflections, setWeeklyReflections, focusItems, focusDone, setTab, setViewDate }) {
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Card, { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => shiftWeek(-1), style: { background: "none", border: "none", color: COLORS.green, fontSize: 18, cursor: "pointer", padding: "4px 8px" } }, "\u2039"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, fontWeight: 700, color: COLORS.ink } }, fmtDate(days[0]), " \u2013 ", fmtDate(days[6])), /* @__PURE__ */ React.createElement("button", { onClick: () => shiftWeek(1), style: { background: "none", border: "none", color: COLORS.green, fontSize: 18, cursor: "pointer", padding: "4px 8px" } }, "\u203A")), weekStart !== mondayOfWeekISO(today) && /* @__PURE__ */ React.createElement("button", { onClick: () => setWeekStart(mondayOfWeekISO(today)), style: { background: "none", border: "none", color: COLORS.clay, fontSize: 12, cursor: "pointer", padding: 0, marginBottom: 10 } }, "back to this week"), focusItems.length > 0 && /* @__PURE__ */ React.createElement(Card, { style: { background: COLORS.claySoft, borderColor: COLORS.clay } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10.5, fontWeight: 700, color: COLORS.clay, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 6 } }, "Focus"), focusItems.map((f) => {
      ...
      const { regularSession } = computeFitnessForDate(d, fitnessSettings, fitnessOverrides);
      const appts = (todos || []).filter((t) => t.date === d && (t.source === "calendar" || t.source === "calendar-live" || t.source === "retreat")).sort((a, b) => apptSortKey(a.text).localeCompare(apptSortKey(b.text)));
  ...
  function trendChange(rows, field) {
    const withValue = rows.filter((r) => r[field] !== void 0 && r[field] !== null).slice().sort((a, b) => a.date.localeCompare(b.date));
    if (withValue.length < 2) return null;
    const first = withValue[0];
    const last = withValue[withValue.length - 1];
    return { firstVal: first[field], lastVal: last[field], firstDate: first.date, lastDate: last.date, change: last[field] - first[field] };
  }
  function TrendChangeLine({ rows, field, unit, label }) {
    const c = trendChange(rows, field);
    if (!c) return null;
    const arrow = c.change > 0 ? "\u25B2" : c.change < 0 ? "\u25BC" : "\u2013";
    const sign = c.change > 0 ? "+" : "";
    return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: COLORS.inkSoft, marginBottom: 6 } }, label ? `${label}: ` : "", arrow, " ", sign, c.change.toFixed(1), unit, " since ", fmtDate(c.firstDate), " (", c.firstVal, unit, " \u2192 ", c.lastVal, unit, ")");
  }
  ...
  function FitnessTab({ settings, setSettings, fitnessOverrides, setFitnessOverrides, fitnessChangeLog, setFitnessChangeLog, logs, setLogs, ironLogs, setIronLogs, ironEquipment, setIronEquipment, ironSubstitutions, setIronSubstitutions, ironNotes, setIronNotes, today, isRetreatDay, regularSession, block, blockIdx, weekNum, dayInBlock }) {
    ...
    const [showC25KPlan, setShowC25KPlan] = useState(false);
    const [openBlockOverview, setOpenBlockOverview] = useState(null);
    const [logDate, setLogDate] = useState(today);
    const logDateIsRetreat = RETREAT_DATES.includes(logDate);
    const logDateInfo = computeFitnessForDate(logDate, settings, fitnessOverrides);
    const retreat = logDateIsRetreat ? RETREAT[logDate] : null;
    const sessionLabel = retreat ? retreat.training : logDateInfo.regularSession;
    const isOverridden = !retreat && logDateInfo.isOverridden;
    ...
    const logToday = (status = "done") => {
      const next = [...logs.filter((l) => l.date !== logDate), { date: logDate, session: sessionLabel, note, status }];
      setLogs(next.sort((a, b) => a.date.localeCompare(b.date)));
      setNote("");
    };
    const todayEntry = logs.find((l) => l.date === logDate);
    ...
    const saveExercise = (name, weight) => {
      if (!weight) return;
      const hist = ironLogs[name] || [];
      const withoutLogDate = hist.filter((h) => h.date !== logDate);
      const next = [...withoutLogDate, { date: logDate, weight }].sort((a, b) => a.date.localeCompare(b.date));
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, logDate === today ? "Today's session" : `Session for ${fmtDate(logDate)}`), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 10, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "Logging for"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: logDate, onChange: (e) => setLogDate(e.target.value), style: { width: 150 } }), logDate !== today && /* @__PURE__ */ React.createElement("button", { onClick: () => setLogDate(today), style: { background: "none", border: "none", color: COLORS.clay, cursor: "pointer", fontSize: 12 } }, "back to today")), /* @__PURE__ */ React.createElement(Pill, null, retreat ? "Retreat override" : `${logDateInfo.block.name} \xB7 Week ${logDateInfo.weekNum}`), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 15, color: COLORS.ink } }, sessionLabel), isOverridden && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, fontSize: 11.5, color: COLORS.clay } }, "Changed from the regular rotation."), !retreat && (PILATES_ALL_OPTIONS.includes(sessionLabel) || sessionLabel === PILATES_PRE_ROTATION_FALLBACK) && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 4 } }, "Change this day's Pilates session"), /* @__PURE__ */ React.createElement(
      Select,
      {
        value: sessionLabel,
        onChange: (e) => {
          if (e.target.value === sessionLabel) return;
          setFitnessOverrides({ ...fitnessOverrides, [logDate]: e.target.value });
          logChange(`Changed ${fmtDate(logDate)}'s Pilates session from "${sessionLabel}" to "${e.target.value}"`);
        },
        style: { minWidth: 220 }
      },
      /* @__PURE__ */ React.createElement("option", { value: PILATES_PRE_ROTATION_FALLBACK }, PILATES_PRE_ROTATION_FALLBACK),
      /* @__PURE__ */ React.createElement("optgroup", { label: "Level 4" }, PILATES_LEVEL4_SERIES.map((s) => /* @__PURE__ */ React.createElement("option", { key: s, value: s }, s))),
    ...
    )), !doneToday && !skippedToday ? /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10 } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "Notes (how it felt overall)", value: note, onChange: (e) => setNote(e.target.value), style: { width: "100%", marginBottom: 8 } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, /* @__PURE__ */ React.createElement(Btn, { onClick: () => logToday("done") }, "Log done"), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: () => logToday("skipped") }, "Skip (come back later)"))) : doneToday ? /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 13, color: COLORS.green } }, "\u2713 Logged for ", logDate === today ? "today" : fmtDate(logDate)) : /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 13, color: COLORS.clay, display: "flex", alignItems: "center", gap: 10 } }, /* @__PURE__ */ React.createElement("span", null, "\u26A0 Skipped \u2014 on your list to revisit"), /* @__PURE__ */ React.createElement("button", { onClick: () => logToday("done"), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12.5, fontWeight: 600 } }, "mark done now"))), skippedSessions.length > 0 && /* @__PURE__ */ React.createElement(Card, { style: { borderColor: COLORS.gold, background: COLORS.claySoft } }, /* @__PURE__ */ React.createElement(SectionTitle, null, "Skipped \u2014 come back to these"), skippedSessions.map((l) => /* @__PURE__ */ React.createElement("div", { key: l.date, style: { display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, fmtDate(l.date)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.ink } }, l.session), l.note && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginTop: 2 } }, l.note)), /* @__PURE__ */ React.createElement("button", { onClick: () => markSkippedDone(l.date), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12, fontWeight: 600 } }, "mark done"), /* @__PURE__ */ React.createElement("button", { onClick: () => dismissSkipped(l.date), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "dismiss")))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Training blocks"), BLOCKS.map((b, bi) => {
      const open = openBlockOverview === bi;
      const isCurrent = bi === blockIdx;
      return /* @__PURE__ */ React.createElement("div", { key: b.name, style: { borderBottom: `1px solid ${COLORS.line}`, padding: "8px 0" } }, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: !open, onToggle: () => setOpenBlockOverview(open ? null : bi) }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, flex: 1 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 700, color: isCurrent ? COLORS.green : COLORS.ink } }, b.name), isCurrent && /* @__PURE__ */ React.createElement(Pill, null, "current"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginLeft: "auto" } }, b.weeks))), open && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, paddingLeft: 18 } }, b.days.map((d, di) => /* @__PURE__ */ React.createElement("div", { key: di, style: { display: "flex", gap: 8, padding: "3px 0", fontSize: 13 } }, /* @__PURE__ */ React.createElement("span", { style: { width: 34, flexShrink: 0, color: COLORS.inkSoft, fontWeight: 600 } }, WEEKDAY_NAMES[di === 6 ? 0 : di + 1].slice(0, 3)), /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.ink } }, d === "__PILATES_ROTATION__" ? "Pilates \u2014 Nell Hoess (Level 4/5/6 rotation)" : d === "__C25K_ROTATION__" ? "C25K progressive run + same-day stretch/yoga (see Couch to 5K plan \u2014 varies weekly)" : d === "__IRON_BLOCK1__" ? "Iron Series \u2014 Caroline Girvan (Mon/Wed/Fri, Days 1\u201330 over 6 weeks \u2014 see full plan)" : d === "__CYCLING_BLOCK2__" ? 'Cycling \u2014 ROUVY "Level Up" 6-week plan (Mon/Wed/Fri \u2014 see full plan)' : d === "__IRON_BLOCK2_FULLBODY__" ? "Iron Series Full Body \u2014 same day each week, see full plan" : d))), b.days.includes("__C25K_ROTATION__") && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, paddingTop: 8, borderTop: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setShowC25KPlan(!showC25KPlan), style: { background: "none", border: "none", color: COLORS.green, fontSize: 12.5, fontWeight: 600, cursor: "pointer", padding: 0 } }, showC25KPlan ? "\u25BE" : "\u25B8", " View full Couch to 5K plan (9 weeks)"), showC25KPlan && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8 } }, C25K_PROGRAM.map((wk, wi) => /* @__PURE__ */ React.createElement("div", { key: wi, style: { marginBottom: 10, paddingBottom: 8, borderBottom: wi < C25K_PROGRAM.length - 1 ? `1px solid ${COLORS.line}` : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12.5, fontWeight: 700, color: COLORS.ink, marginBottom: 3 } }, "Week ", wi + 1), Array.isArray(wk) ? wk.map((dayText, di2) => /* @__PURE__ */ React.createElement("div", { key: di2, style: { fontSize: 12, color: COLORS.inkSoft, marginBottom: 2 } }, /* @__PURE__ */ React.createElement("b", { style: { color: COLORS.ink } }, ["Mon", "Wed", "Fri"][di2], ":"), " ", dayText, " ", /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.clay } }, "+ ", RUN_DAY_FOLLOWUP[di2]))) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: COLORS.inkSoft, marginBottom: 2 } }, /* @__PURE__ */ React.createElement("b", { style: { color: COLORS.ink } }, "Mon:"), " ", wk, " ", /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.clay } }, "+ ", RUN_DAY_FOLLOWUP[0])), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: COLORS.inkSoft, marginBottom: 2 } }, /* @__PURE__ */ React.createElement("b", { style: { color: COLORS.ink } }, "Wed:"), " ", wk, " ", /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.clay } }, "+ ", RUN_DAY_FOLLOWUP[1])), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: COLORS.inkSoft, marginBottom: 2 } }, /* @__PURE__ */ React.createElement("b", { style: { color: COLORS.ink } }, "Fri:"), " ", wk, " ", /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.clay } }, "+ ", RUN_DAY_FOLLOWUP[2])))))))));
    })), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Swap two days"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end", marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 3 } }, "Date A"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: swapDateA, onChange: (e) => setSwapDateA(e.target.value) }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginTop: 3, maxWidth: 150 } }, sessionForDate(swapDateA))), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 3 } }, "Date B"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: swapDateB, onChange: (e) => setSwapDateB(e.target.value) }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginTop: 3, maxWidth: 150 } }, sessionForDate(swapDateB))), /* @__PURE__ */ React.createElement(Btn, { onClick: swapTwoDates }, "Swap")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginTop: 16, marginBottom: 4 } }, "Or just change one day (no second date affected)"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" } }, /* @__PURE__ */ React.createElement(Input, { type: "date", value: singleDate, onChange: (e) => setSingleDate(e.target.value) }), /* @__PURE__ */ React.createElement(Select, { value: singleSessionChoice, onChange: (e) => setSingleSessionChoice(e.target.value), style: { minWidth: 200 } }, /* @__PURE__ */ React.createElement("option", { value: "" }, "\u2014 Reset to scheduled \u2014"), BLOCKS.map((b) => /* @__PURE__ */ React.createElement("optgroup", { key: b.name, label: b.name }, b.days.flatMap((d) => d === "__PILATES_ROTATION__" ? PILATES_ALL_OPTIONS : d === "__C25K_ROTATION__" ? C25K_PROGRAM.flatMap((w) => Array.isArray(w) ? w : [w]) : d === "__IRON_BLOCK1__" || d === "__IRON_BLOCK2_FULLBODY__" ? Object.keys(IRON_DAYS).map((n) => `Iron Series Day ${n} \u2014 ${IRON_DAYS[n].title} (30 min)`) : d === "__CYCLING_BLOCK2__" ? ROUVY_LEVEL_UP.flat() : [d]).map((d) => /* @__PURE__ */ React.createElement("option", { key: d, value: d }, d))))), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: applySingleOverride }, "Save")), overrideDates.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 16, paddingTop: 12, borderTop: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 6 } }, "Active swaps / overrides"), overrideDates.map((d) => /* @__PURE__ */ React.createElement("div", { key: d, style: { display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, fmtDate(d)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: COLORS.ink } }, fitnessOverrides[d])), /* @__PURE__ */ React.createElement("button", { onClick: () => clearOverride(d), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "reset"))))), isIronSession && /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Weeks 1\u20136 exercise reference \u2014 pick the Iron day this session matches." }, "Iron Series exercises"), /* @__PURE__ */ React.createElement(Select, { value: ironDay, onChange: (e) => setIronDay(parseInt(e.target.value)), style: { marginBottom: 10 } }, Object.entries(IRON_DAYS).map(([k, v]) => /* @__PURE__ */ React.createElement("option", { key: k, value: k }, "Day ", k, " \u2014 ", v.title))), ironData.exercises.map((ex, i) => {
      ...
      const todayEntry2 = (ironLogs[name] || []).find((h) => h.date === logDate);
      const hist = ironLogs[name] || [];
      ...
      return /* @__PURE__ */ React.createElement("div", { key: ironDay + "-" + i + "-" + logDate, style: { padding: "8px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 6 } }, /* @__PURE__ */ React.createElement("div", null, editingSub ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(
        Input,
        {
          value: subInputValue,
      ...
      ), historyOpen && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, paddingLeft: 8, borderLeft: `2px solid ${COLORS.line}` } }, (() => {
        const chartData = [...hist].map((h) => ({ date: h.date, weight: parseFloat(h.weight) })).filter((h) => !isNaN(h.weight)).sort((a, b) => a.date.localeCompare(b.date)).map((h) => ({ date: h.date.slice(5), weight: h.weight }));
        return chartData.length > 1 ? /* @__PURE__ */ React.createElement("div", { style: { width: "100%", height: 140, marginBottom: 8 } }, /* @__PURE__ */ React.createElement(ResponsiveContainer, null, /* @__PURE__ */ React.createElement(LineChart, { data: chartData }, /* @__PURE__ */ React.createElement(CartesianGrid, { stroke: COLORS.line }), /* @__PURE__ */ React.createElement(XAxis, { dataKey: "date", fontSize: 10, stroke: COLORS.inkSoft }), /* @__PURE__ */ React.createElement(YAxis, { fontSize: 10, stroke: COLORS.inkSoft, domain: ["dataMin - 2", "dataMax + 2"] }), /* @__PURE__ */ React.createElement(Tooltip, null), /* @__PURE__ */ React.createElement(Line, { type: "monotone", dataKey: "weight", stroke: COLORS.green, strokeWidth: 2, dot: { r: 3 }, connectNulls: true })))) : null;
      })(), [...hist].reverse().map((h, hi) => /* @__PURE__ */ React.createElement("div", { key: hi, style: { fontSize: 11.5, color: COLORS.inkSoft, padding: "2px 0" } }, fmtDate(h.date), ": ", h.weight))));
  ...
  function SkinTab({ today, isRetreatDay, ziipSettings, setZiipSettings, ziipToday, ziipDayIdx, ziipPlanName, ziipPlanLength, ziipDone, setZiipDone, amDone, setAmDone, pmDone, setPmDone, skinPlan, setSkinPlan, ledMaskDone, setLedMaskDone, skinPurchases, setSkinPurchases }) {
    ...
    const [newPurchaseText, setNewPurchaseText] = useState("");
    const addSkinPurchase = () => {
      if (!newPurchaseText.trim()) return;
      setSkinPurchases([...skinPurchases, { id: "sp-" + Date.now(), text: newPurchaseText.trim(), done: false }]);
      setNewPurchaseText("");
    };
    const toggleSkinPurchase = (id) => setSkinPurchases(skinPurchases.map((p) => p.id === id ? { ...p, done: !p.done } : p));
    const removeSkinPurchase = (id) => setSkinPurchases(skinPurchases.filter((p) => p.id !== id));
    ...
    return /* @__PURE__ */ React.createElement("div", null, retreat && /* @__PURE__ */ React.createElement(Card, { style: { borderColor: COLORS.gold, background: COLORS.claySoft } }, /* @__PURE__ */ React.createElement(Pill, { tone: "clay" }, "Post-sun recovery week"), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, fontSize: 13.5, color: COLORS.ink } }, retreat.skinNote), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, fontSize: 12.5, color: COLORS.clay } }, "No at-home actives (retinol/exfoliants/Vit C) this week \u2014 hydrating serum + moisturiser AM and PM is enough. Normal weekly rotation resumes after.")), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: `Day ${ziipDayIdx + 1} of ${ziipPlanLength}` }, "ZIIP Halo \u2014 ", ziipPlanName), /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12.5, color: COLORS.inkSoft } }, "Plan start date: "), /* @__PURE__ */ React.createElement(Input, { type: "date", value: ziipSettings.startDate, onChange: (e) => setZiipSettings({ startDate: e.target.value }), style: { marginLeft: 4 } })), !ziipToday ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.inkSoft } }, "30-day plan complete \u2014 update the start date to begin a new cycle.") : ziipToday.rest ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: COLORS.inkSoft } }, "Rest day \u2014 no treatments.") : ziipToday.t.map(([name, mins]) => /* @__PURE__ */ React.createElement(
      Check,
      {
        key: name,
    ...
    )), isNonZiipDay && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, paddingTop: 10, borderTop: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement(Check, { checked: ledMaskDoneToday, onChange: toggleLedMask, label: "LED mask" }))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Skin care purchases"), skinPurchases.filter((p) => !p.done).length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: COLORS.inkSoft, marginBottom: 8 } }, "Nothing on the list right now.") : skinPurchases.filter((p) => !p.done).map((p) => /* @__PURE__ */ React.createElement("div", { key: p.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "5px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: p.done, onChange: () => toggleSkinPurchase(p.id), style: { accentColor: COLORS.clay, width: 16, height: 16, flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 14, color: COLORS.ink } }, p.text), /* @__PURE__ */ React.createElement("button", { onClick: () => removeSkinPurchase(p.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, marginTop: 10 } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "e.g. Replace SK-II First Essence", value: newPurchaseText, onChange: (e) => setNewPurchaseText(e.target.value), onKeyDown: (e) => {
      if (e.key === "Enter") addSkinPurchase();
    }, style: { flex: 1 } }), /* @__PURE__ */ React.createElement(Btn, { onClick: addSkinPurchase }, "Add")), skinPurchases.some((p) => p.done) && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, paddingTop: 8, borderTop: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 4 } }, "Bought"), skinPurchases.filter((p) => p.done).map((p) => /* @__PURE__ */ React.createElement("div", { key: p.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "4px 0" } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: p.done, onChange: () => toggleSkinPurchase(p.id), style: { accentColor: COLORS.clay, width: 16, height: 16, flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 13, color: COLORS.inkSoft, textDecoration: "line-through" } }, p.text), /* @__PURE__ */ React.createElement("button", { onClick: () => removeSkinPurchase(p.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"))))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: schedule.label }, "AM Routine"), retreat ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.inkSoft } }, "Recovery week: cleanse \xB7 hydrating serum \xB7 moisturiser \xB7 eye cream. No Vit C / actives.") : schedule.am.map((step) => /* @__PURE__ */ React.createElement(
      Check,
      {
        key: step.id,
  ...
  function normalizeIngredientName(name) {
    ...
    n = n.replace(/\([^)]*\)/g, "").trim();
    n = n.split(",")[0].trim();
    const trailingQtyPattern = /\s+(\d+\.?\d*\/?\d*\s*)?(g|kg|ml|l|tsp|tbsp|cups?|cloves?|oz|lbs?)\.?\s*$/i;
    n = n.replace(trailingQtyPattern, "").trim();
    n = n.replace(trailingQtyPattern, "").trim();
  ...
  var SHOPPING_CATEGORIES = [
    {
      name: "Tinned & Canned",
      keywords: ["tinned", "canned", "tin of", "can of"]
    },
    {
      name: "Frozen",
      keywords: ["frozen"]
    ...
    {
      name: "Beverages",
      keywords: ["juice", "coffee", "tea bag", "wine", "soda water", "kombucha", "sparkling water", "cordial", "hydrolyte"]
    },
    {
      name: "Bakery",
      keywords: ["bread", "tortilla", "wrap", "bun", "bread roll", "bagel", "pita", "baguette", "croissant"]
    },
    {
      name: "Condiments & Sauces",
      keywords: ["sauce", "mustard", "mayo", "ketchup", "dressing", "paste", "salsa", "chutney", "relish", "marinade"]
    },
    {
      // Checked before Dairy and Fresh Produce so a modified/dried form of an
      // otherwise-fresh or dairy word (coconut milk vs milk, garlic powder vs
      // garlic) matches its more specific phrase here first.
      name: "Pantry & Dry Goods",
      keywords: ["garlic powder", "onion powder", "ginger powder", "dried basil", "dried oregano", "dried herb", "coconut milk", "coconut cream", "rice", "pasta", "flour", "sugar", "oats", "quinoa", "lentil", "chickpea", "stock", "broth", "olive oil", "sesame oil", "vegetable oil", "cooking oil", "vinegar", "spice", "cumin", "paprika", "cinnamon", "salt", "black pepper", "honey", "almond", "cashew", "peanut", "walnut", "arborio", "noodle", "cereal", "muesli", "breadcrumb", "tahini", "maple syrup", "vanilla", "baking powder", "bicarb", "yeast", "black bean", "kidney bean", "cannellini"]
    },
    {
      name: "Dairy & Eggs",
      keywords: ["milk", "cheese", "yoghurt", "yogurt", "butter", "cream", "egg", "parmesan", "mozzarella", "feta", "ricotta", "cheddar", "haloumi", "halloumi", "kefir", "custard"]
    },
    {
      name: "Meat, Poultry & Seafood",
      keywords: ["chicken", "beef", "pork", "lamb", "turkey", "bacon", "sausage", "mince", "steak", "salmon", "tuna", "fish", "prawn", "shrimp", "octopus", "squid", "duck", "veal", "ham", "mussel", "oyster", "crab", "lobster", "trout", "cod", "snapper", "venison"]
    },
    {
      name: "Fresh Produce",
      keywords: ["tomato", "onion", "garlic", "carrot", "potato", "broccolini", "broccoli", "spinach", "lettuce", "cucumber", "zucchini", "capsicum", "bell pepper", "avocado", "apple", "banana", "lemon", "lime", "orange", "ginger", "mushroom", "celery", "kale", "cabbage", "pumpkin", "corn", "asparagus", "basil", "coriander", "cilantro", "parsley", "mint leaves", "fresh chilli", "chive", "shallot", "leek", "beetroot", "radish", "cauliflower", "green bean", "snow pea", "sugar snap", "bean sprout", "fresh herb", "salad", "berry", "grape", "pear", "peach", "mango", "watermelon", "pineapple", "kiwi fruit", "edamame"]
    }
  ];
  ...
  var SHOPPING_CATEGORY_DISPLAY_ORDER = ["Fresh Produce", "Meat, Poultry & Seafood", "Dairy & Eggs", "Bakery", "Pantry & Dry Goods", "Tinned & Canned", "Frozen", "Condiments & Sauces", "Beverages", "Other"];
  function getSlotItems(dayEntry, slot) {
    if (!dayEntry) return [];
    if (dayEntry[slot + "Items"]) return dayEntry[slot + "Items"];
    const recipeId = dayEntry[slot + "RecipeId"];
    const hasText = !!dayEntry[slot];
    if (!hasText) return [];
    if (recipeId) return [{ id: "legacy-" + slot, recipeId, servings: dayEntry[slot + "Servings"] || 1 }];
    return [{ id: "legacy-" + slot, recipeId: null, customText: dayEntry[slot], servings: 1 }];
  }
  function mealSlotDisplayText(items, recipes) {
    return items.map((it) => it.recipeId ? (recipes.find((r) => r.id === it.recipeId) || {}).name : it.customText).filter(Boolean).join(", ");
  }
  function dayNutritionTotals(dayEntry, recipes) {
    ...
    ["breakfast", "lunch", "dinner", "snacks"].forEach((slot) => {
      const items = getSlotItems(dayEntry, slot);
      ...
      items.forEach((it) => {
        ...
        totals.fiber += (r.fiber || 0) * servings;
        totals.calcium += (r.calcium || 0) * servings;
      });
      if (slotLogged) loggedSlots++;
    });
    (dayEntry.extraLogs || []).forEach((ex) => {
      totals.calories += ex.calories || 0;
      totals.protein += ex.protein || 0;
      totals.carbs += ex.carbs || 0;
      totals.fat += ex.fat || 0;
      totals.fiber += ex.fiber || 0;
      totals.calcium += ex.calcium || 0;
    });
  ...
  function MealsTab({ today, meals, setMeals, recipes, setRecipes, shoppingList, setShoppingList, nutritionTargets, setNutritionTargets }) {
    ...
    const addSlotItem = (slot, item) => {
      const items = [...getSlotItems(entry, slot), { id: "item-" + Date.now(), ...item }];
      const next = { ...entry, [slot + "Items"]: items, [slot]: mealSlotDisplayText(items, recipes) };
      delete next[slot + "RecipeId"];
      delete next[slot + "Servings"];
      setMeals({ ...meals, [day]: next });
    };
    const removeSlotItem = (slot, itemId) => {
      const items = getSlotItems(entry, slot).filter((it) => it.id !== itemId);
      setMeals({ ...meals, [day]: { ...entry, [slot + "Items"]: items, [slot]: mealSlotDisplayText(items, recipes) } });
    };
    const updateSlotItemServings = (slot, itemId, servings) => {
      const items = getSlotItems(entry, slot).map((it) => it.id === itemId ? { ...it, servings } : it);
      setMeals({ ...meals, [day]: { ...entry, [slot + "Items"]: items } });
    };
    const [copyFromDate, setCopyFromDate] = useState(day);
    const [copySourceSlot, setCopySourceSlot] = useState("breakfast");
    const [copyTargetSlot, setCopyTargetSlot] = useState("breakfast");
    const [copyMessage, setCopyMessage] = useState("");
    const cloneItems = (items) => items.map((it) => ({ ...it, id: "item-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7) }));
    const copyMealSlot = () => {
      if (copyFromDate === day && copySourceSlot === copyTargetSlot) return;
      const sourceEntry = meals[copyFromDate];
      if (!sourceEntry) {
        setCopyMessage(`Nothing found for ${fmtDate(copyFromDate)}.`);
        return;
      }
      const items = cloneItems(getSlotItems(sourceEntry, copySourceSlot));
      setMeals({ ...meals, [day]: { ...entry, [copyTargetSlot + "Items"]: items, [copyTargetSlot]: mealSlotDisplayText(items, recipes) } });
      setCopyMessage(`Copied ${copySourceSlot} from ${fmtDate(copyFromDate)} into ${copyTargetSlot} on ${fmtDate(day)}.`);
    };
    const copyEntireDay = () => {
      if (copyFromDate === day) return;
      const sourceEntry = meals[copyFromDate];
      if (!sourceEntry) {
        setCopyMessage(`Nothing found for ${fmtDate(copyFromDate)}.`);
        return;
      }
      const next = { ...entry, notes: sourceEntry.notes || "" };
      ["breakfast", "lunch", "dinner", "snacks"].forEach((slot) => {
        const items = cloneItems(getSlotItems(sourceEntry, slot));
        next[slot + "Items"] = items;
        next[slot] = mealSlotDisplayText(items, recipes);
        delete next[slot + "RecipeId"];
        delete next[slot + "Servings"];
      });
      setMeals({ ...meals, [day]: next });
      setCopyMessage(`Copied all of ${fmtDate(copyFromDate)} into ${fmtDate(day)}.`);
    };
    const [bulkSlot, setBulkSlot] = useState("breakfast");
    const [bulkRecipeChoice, setBulkRecipeChoice] = useState("__custom__");
    const [bulkCustomText, setBulkCustomText] = useState("");
    const [bulkServings, setBulkServings] = useState(1);
    const [bulkStartDate, setBulkStartDate] = useState(today);
    const [bulkDays, setBulkDays] = useState("7");
    const [bulkMessage, setBulkMessage] = useState("");
    const applyBulkMealFill = () => {
      const n = Math.max(1, parseInt(bulkDays, 10) || 1);
      const recipe = bulkRecipeChoice !== "__custom__" ? recipes.find((r) => r.id === bulkRecipeChoice) : null;
      const text = recipe ? recipe.name : bulkCustomText.trim();
      if (!text) return;
      const item = recipe ? { id: "item-" + Date.now(), recipeId: recipe.id, servings: parseFloat(bulkServings) || 1 } : { id: "item-" + Date.now(), recipeId: null, customText: text };
      const updates = {};
      for (let i = 0; i < n; i++) {
        const d = addDaysISO(bulkStartDate, i);
        const existing = meals[d] || {};
        const next = { ...existing, [bulkSlot + "Items"]: [item], [bulkSlot]: text };
        delete next[bulkSlot + "RecipeId"];
        delete next[bulkSlot + "Servings"];
        updates[d] = next;
      }
      setMeals({ ...meals, ...updates });
      setBulkMessage(`Set for ${n} day${n === 1 ? "" : "s"}, from ${fmtDate(bulkStartDate)}.`);
    };
    const [collapsedSections, setCollapsedSections] = useState({});
    const isCollapsed = (key) => collapsedSections[key] !== false;
    const toggleSection = (key) => setCollapsedSections({ ...collapsedSections, [key]: !isCollapsed(key) });
    const [showAddRecipe, setShowAddRecipe] = useState(false);
    const [newRecipeType, setNewRecipeType] = useState("recipe");
    ...
    const [editRecipeType, setEditRecipeType] = useState("recipe");
    ...
    const saveNewRecipe = () => {
      ...
      const recipe = { id: "recipe-" + Date.now(), type: newRecipeType, name: newRecipeName, servings: newServings ? parseFloat(newServings) : null, ingredients, instructions: newInstructions };
      if (newCalories) recipe.calories = clampMacroValue("calories", parseFloat(newCalories));
      if (newProtein) recipe.protein = clampMacroValue("protein", parseFloat(newProtein));
      if (newCarbs) recipe.carbs = clampMacroValue("carbs", parseFloat(newCarbs));
      if (newFat) recipe.fat = clampMacroValue("fat", parseFloat(newFat));
      if (newFiber) recipe.fiber = clampMacroValue("fiber", parseFloat(newFiber));
      if (newCalcium) recipe.calcium = clampMacroValue("calcium", parseFloat(newCalcium));
      setRecipes([...recipes, recipe]);
      setNewRecipeType("recipe");
      setNewRecipeName("");
      setNewServings("");
    ...
    const startEditRecipe = (r) => {
      ...
      setEditRecipeType(r.type || "recipe");
      setEditName(r.name);
      ...
      setEditFiber(r.fiber != null ? String(r.fiber) : "");
      setEditCalcium(r.calcium != null ? String(r.calcium) : "");
    ...
    const saveEditRecipe = () => {
      ...
      setRecipes(recipes.map((r) => r.id === editRecipeId ? {
        ...
        calories: editCalories ? clampMacroValue("calories", parseFloat(editCalories)) : void 0,
        protein: editProtein ? clampMacroValue("protein", parseFloat(editProtein)) : void 0,
        carbs: editCarbs ? clampMacroValue("carbs", parseFloat(editCarbs)) : void 0,
        fat: editFat ? clampMacroValue("fat", parseFloat(editFat)) : void 0,
        fiber: editFiber ? clampMacroValue("fiber", parseFloat(editFiber)) : void 0,
        calcium: editCalcium ? clampMacroValue("calcium", parseFloat(editCalcium)) : void 0
      } : r));
    ...
    const parsePastedRecipe = () => {
      ...
      const name = rawLines[0] || "Untitled recipe";
      const stripBullet = (l) => l.replace(/^[•\-*▪◦]\s*/, "").trim();
      const ingredients = [];
      const instructionLines = [];
      let servingsFromText = null;
      const macros = {};
      ...
      const macroPatterns = {
        ...
        fat: /(\d+\.?\d*)\s*g\s*fat/i,
        fiber: /(\d+\.?\d*)\s*g\s*fib(?:re|er)/i,
        calcium: /~?\s*(\d+\.?\d*)\s*mg\s*calcium/i
      };
      for (let i = 1; i < rawLines.length; i++) {
        ...
        if (/^(method|instructions?|directions?)\s*:?$/i.test(line)) {
          section = "instructions";
          continue;
        }
        const servesMatch = line.match(/^serves:?\s*(\d+)/i);
        if (servesMatch) {
          servingsFromText = parseInt(servesMatch[1], 10);
          continue;
        }
        if (/kcal/i.test(line)) {
          Object.entries(macroPatterns).forEach(([key, re]) => {
            const m = line.match(re);
            if (m) macros[key] = parseFloat(m[1]);
          });
          continue;
        }
        if (/^\d+\.\s/.test(line)) {
        ...
        const qtyFirst = line.match(/^(\d+\.?\d*)\s*([a-zA-Z%]*)\s+(.+)$/);
        const nameDashQty = line.match(/^(.+?)\s+[–—-]\s+(.+)$/);
        if (qtyFirst) {
          ingredients.push({ name: qtyFirst[3], qty: parseFloat(qtyFirst[1]) || 1, unit: qtyFirst[2] || "" });
        } else if (nameDashQty) {
          const ingName = nameDashQty[1];
          const qtyPart = nameDashQty[2];
          const qtyMatch = qtyPart.match(/^(\d+\.?\d*)\s*([a-zA-Z]*)/);
          if (qtyMatch && qtyMatch[1]) {
            ingredients.push({ name: ingName, qty: parseFloat(qtyMatch[1]) || 1, unit: qtyMatch[2] || "" });
          } else {
            ingredients.push({ name: ingName, qty: 1, unit: qtyPart });
          }
        } else if (hadBullet || sawIngredientsHeader) {
      ...
      const recipe = { id: "recipe-" + Date.now(), type: "recipe", name, ingredients, instructions: instructionLines.join(" ") };
      if (servingsFromText) recipe.servings = servingsFromText;
      if (macros.calories !== void 0) recipe.calories = clampMacroValue("calories", macros.calories);
      if (macros.protein !== void 0) recipe.protein = clampMacroValue("protein", macros.protein);
      if (macros.carbs !== void 0) recipe.carbs = clampMacroValue("carbs", macros.carbs);
      if (macros.fat !== void 0) recipe.fat = clampMacroValue("fat", macros.fat);
      if (macros.fiber !== void 0) recipe.fiber = clampMacroValue("fiber", macros.fiber);
      if (macros.calcium !== void 0) recipe.calcium = clampMacroValue("calcium", macros.calcium);
      setRecipes([...recipes, recipe]);
      setPasteText("");
    };
    const removeRecipe = (id) => setRecipes(recipes.filter((r) => r.id !== id));
    const toggleRecipeType = (id) => setRecipes(recipes.map((r) => r.id === id ? { ...r, type: r.type === "item" ? "recipe" : "item" } : r));
    const renderRecipeRow = (r) => {
      const open = openRecipeId === r.id;
      const editing = editRecipeId === r.id;
      return /* @__PURE__ */ React.createElement("div", { key: r.id, style: { padding: "8px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setOpenRecipeId(open ? null : r.id), style: { background: "none", border: "none", color: COLORS.ink, fontWeight: 600, fontSize: 14, cursor: "pointer", padding: 0, textAlign: "left" } }, r.name, " ", /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.inkSoft, fontWeight: 400, fontSize: 12 } }, "(", r.ingredients.length, " ingredients", r.servings ? `, makes ${r.servings} serving${r.servings === 1 ? "" : "s"}` : "", ")"), r.calories != null && /* @__PURE__ */ React.createElement("div", { style: { color: COLORS.clay, fontWeight: 600, fontSize: 11.5, marginTop: 2 } }, r.calories, " kcal", r.protein != null ? ` \xB7 ${r.protein}g protein` : "", r.carbs != null ? ` \xB7 ${r.carbs}g carbs` : "", r.fat != null ? ` \xB7 ${r.fat}g fat` : "", r.fiber != null ? ` \xB7 ${r.fiber}g fibre` : "", r.calcium != null ? ` \xB7 ${r.calcium}mg calcium` : "", " ", /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.inkSoft, fontWeight: 400 } }, "per serving"))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10 } }, !editing && /* @__PURE__ */ React.createElement("button", { onClick: () => toggleRecipeType(r.id), style: { background: "none", border: "none", color: COLORS.slate, cursor: "pointer", fontSize: 11.5 } }, r.type === "item" ? "\u2192 Recipes" : "\u2192 Items"), /* @__PURE__ */ React.createElement("button", { onClick: () => editing ? cancelEditRecipe() : startEditRecipe(r), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12 } }, editing ? "cancel" : "edit"), /* @__PURE__ */ React.createElement("button", { onClick: () => removeRecipe(r.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"))), open && editing && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, padding: 10, background: COLORS.greenSoft, borderRadius: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, marginBottom: 8 } }, /* @__PURE__ */ React.createElement(Select, { value: editRecipeType, onChange: (e) => setEditRecipeType(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "recipe" }, "Recipe"), /* @__PURE__ */ React.createElement("option", { value: "item" }, "Individual item")), /* @__PURE__ */ React.createElement(Input, { placeholder: "Recipe name", value: editName, onChange: (e) => setEditName(e.target.value), style: { flex: 1 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "Servings", value: editServings, onChange: (e) => setEditServings(e.target.value), style: { width: 90 } })), editIngredients.map((ing, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { display: "flex", gap: 6, marginBottom: 6 } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "qty", value: ing.qty, onChange: (e) => updateEditIngredientRow(i, "qty", e.target.value), style: { width: 55 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "unit", value: ing.unit, onChange: (e) => updateEditIngredientRow(i, "unit", e.target.value), style: { width: 65 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "ingredient", value: ing.name, onChange: (e) => updateEditIngredientRow(i, "name", e.target.value), style: { flex: 1 } }), /* @__PURE__ */ React.createElement("button", { onClick: () => removeEditIngredientRow(i), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"))), /* @__PURE__ */ React.createElement("button", { onClick: addEditIngredientRow, style: { background: "none", border: "none", color: COLORS.green, fontSize: 12, cursor: "pointer", padding: 0, marginBottom: 8 } }, "+ add ingredient"), /* @__PURE__ */ React.createElement(Input, { placeholder: "Instructions (optional)", value: editInstructions, onChange: (e) => setEditInstructions(e.target.value), style: { width: "100%", marginBottom: 8 } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 4 } }, "Nutrition per serving (optional)"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "kcal", value: editCalories, onChange: (e) => setEditCalories(e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "protein g", value: editProtein, onChange: (e) => setEditProtein(e.target.value), style: { width: 80 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "carbs g", value: editCarbs, onChange: (e) => setEditCarbs(e.target.value), style: { width: 80 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fat g", value: editFat, onChange: (e) => setEditFat(e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fibre g", value: editFiber, onChange: (e) => setEditFiber(e.target.value), style: { width: 75 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "calcium mg", value: editCalcium, onChange: (e) => setEditCalcium(e.target.value), style: { width: 85 } })), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, /* @__PURE__ */ React.createElement(Btn, { onClick: saveEditRecipe }, "Save changes"), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: cancelEditRecipe }, "Cancel"))), open && !editing && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, paddingLeft: 8, borderLeft: `2px solid ${COLORS.line}` } }, r.ingredients.map((ing, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 12.5, color: COLORS.ink } }, ing.qty, " ", ing.unit, " ", ing.name)), r.instructions && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12.5, color: COLORS.inkSoft, marginTop: 6 } }, r.instructions), r.calories == null && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginTop: 6, fontStyle: "italic" } }, "No nutrition info yet \u2014 add it via edit.")));
    };
    ...
    const setTarget = (field, val) => {
      if (val === "") {
        setNutritionTargets({ ...nutritionTargets, [field]: null });
        return;
      }
      const num = parseFloat(val);
      if (isNaN(num)) return;
      const cap = NUTRITION_TARGET_MAX[field] || 1e4;
      setNutritionTargets({ ...nutritionTargets, [field]: Math.min(num, cap) });
    };
    const [quickLogLabel, setQuickLogLabel] = useState("");
    const [quickLogCalories, setQuickLogCalories] = useState("");
    const [quickLogProtein, setQuickLogProtein] = useState("");
    const [quickLogCarbs, setQuickLogCarbs] = useState("");
    const [quickLogFat, setQuickLogFat] = useState("");
    const [quickLogFiber, setQuickLogFiber] = useState("");
    const [quickLogCalcium, setQuickLogCalcium] = useState("");
    const addQuickLog = () => {
      if (!quickLogLabel.trim() || !quickLogCalories) return;
      const item = {
        id: "ql-" + Date.now(),
        label: quickLogLabel.trim(),
        calories: clampMacroValue("calories", parseFloat(quickLogCalories) || 0),
        protein: clampMacroValue("protein", parseFloat(quickLogProtein) || 0),
        carbs: clampMacroValue("carbs", parseFloat(quickLogCarbs) || 0),
        fat: clampMacroValue("fat", parseFloat(quickLogFat) || 0),
        fiber: clampMacroValue("fiber", parseFloat(quickLogFiber) || 0),
        calcium: clampMacroValue("calcium", parseFloat(quickLogCalcium) || 0)
      };
      setMeals({ ...meals, [day]: { ...entry, extraLogs: [...entry.extraLogs || [], item] } });
      const alreadyExists = recipes.some((r) => r.type === "item" && r.name.trim().toLowerCase() === item.label.toLowerCase());
      if (!alreadyExists) {
        setRecipes([...recipes, {
          id: "recipe-" + Date.now(),
          type: "item",
    ...
    const removeQuickLog = (id) => {
      setMeals({ ...meals, [day]: { ...entry, extraLogs: (entry.extraLogs || []).filter((ex) => ex.id !== id) } });
    };
    ...
    const nutTrendData = Object.keys(meals).filter((d) => (!nutTrendStartDate || d >= nutTrendStartDate) && d <= nutTrendEndDate).sort().map((d) => {
      ...
      Object.keys(totals).forEach((k) => {
        rounded[k] = Math.round(totals[k] * 10) / 10;
      });
      return { date: d.slice(5), ...rounded };
    }).filter(Boolean);
    ...
    const generateShoppingList = () => {
      ...
      dates.forEach((d) => {
        ...
        ["breakfast", "lunch", "dinner", "snacks"].forEach((slot) => {
          const items2 = dayEntry ? getSlotItems(dayEntry, slot) : [];
          const unmatchedForSlot = [];
          if (items2.length > 0) {
            items2.forEach((it) => {
              const r = it.recipeId ? recipes.find((rec) => rec.id === it.recipeId) : null;
              const servings = it.servings || 1;
              if (r && r.ingredients && r.ingredients.length > 0) {
                const recipeYield = r.servings || 1;
                const scale = servings / recipeYield;
                r.ingredients.forEach((ing) => {
                  const cleanName = normalizeIngredientName(ing.name);
                  const key = ingredientKey(cleanName, ing.unit);
                  if (!aggregated[key]) aggregated[key] = { name: cleanName, unit: ing.unit, qty: 0 };
                  aggregated[key].qty += ing.qty * scale;
                });
              } else if (r && r.type === "item") {
                const key = ingredientKey(r.name, "");
                if (!aggregated[key]) aggregated[key] = { name: r.name, unit: "", qty: 0 };
                aggregated[key].qty += servings;
              } else {
                unmatchedForSlot.push(r ? r.name : it.customText);
              }
            });
          } else if (dayRetreat) {
            const text = slot !== "breakfast" ? dayRetreat[slot] : RETREAT_CONSTANTS.breakfast;
            if (text) unmatchedForSlot.push(text);
          }
          if (unmatchedForSlot.length > 0) unmatched.push(`${fmtDate(d)} ${slot}: ${unmatchedForSlot.join(", ")}`);
        });
      });
      const items = Object.values(aggregated).sort((a, b) => a.name.localeCompare(b.name)).map((i, idx) => ({ id: "sl-" + idx, name: i.name, qty: Math.round(i.qty * 100) / 100, unit: i.unit, done: false, category: categorizeIngredient(i.name) }));
    ...
    const printShoppingList = () => {
      ...
      const checked = shoppingList.items.filter((i) => i.done);
      if (checked.length === 0) {
        alert("Tick the items you want on this trip first, then print.");
        return;
      }
      const byCategory = {};
      checked.forEach((i) => {
      ...
      let html = `<html><head><title>Shopping List \u2014 ${esc(shoppingList.range)}</title><style>
      body { font-family: -apple-system, Helvetica, Arial, sans-serif; padding: 28px; color: #2B2A28; max-width: 480px; margin: 0 auto; }
      h1 { font-size: 20px; margin: 0 0 2px; }
      .range { font-size: 13px; color: #6B6459; margin-bottom: 18px; }
      h2 { font-size: 12.5px; text-transform: uppercase; letter-spacing: 0.5px; color: #A63A50; margin: 18px 0 6px; border-bottom: 1px solid #E7E0D2; padding-bottom: 4px; }
      .item { font-size: 14px; padding: 4px 0; display: flex; align-items: baseline; gap: 9px; }
      .box { width: 12px; height: 12px; border: 1.5px solid #2B2A28; display: inline-block; flex-shrink: 0; }
      @media print { body { padding: 10px; } }
    </style></head><body>
      <h1>Shopping List</h1>
      <div class="range">${esc(shoppingList.range)}</div>`;
      SHOPPING_CATEGORY_DISPLAY_ORDER.forEach((cat) => {
        const items = byCategory[cat];
        if (!items || items.length === 0) return;
        html += `<h2>${esc(cat)}</h2>`;
        items.forEach((i) => {
          html += `<div class="item"><span class="box"></span><span>${esc(i.qty)} ${esc(i.unit)} ${esc(i.name)}</span></div>`;
        });
      });
      html += `</body></html>`;
    ...
    const downloadShoppingListWord = () => {
      if (!shoppingList) return;
      const checked = shoppingList.items.filter((i) => i.done);
      if (checked.length === 0) {
        alert("Tick the items you want on this trip first, then download.");
        return;
      }
      const byCategory = {};
      checked.forEach((i) => {
      ...
      let body = `{\\b\\fs32 Shopping List}\\par
{\\i\\fs20 ${rtfEsc(shoppingList.range)}}\\par
\\par
`;
      SHOPPING_CATEGORY_DISPLAY_ORDER.forEach((cat) => {
        const items = byCategory[cat];
        if (!items || items.length === 0) return;
        body += `{\\b\\fs24 ${rtfEsc(cat.toUpperCase())}}\\par
`;
        items.forEach((i) => {
          body += `\\bullet\\tab ${rtfEsc(i.qty)} ${rtfEsc(i.unit)} ${rtfEsc(i.name)}\\par
`;
        });
        body += `\\par
`;
      });
      body += `{\\b\\fs24 MY ITEMS}\\par
`;
      for (let i = 0; i < 6; i++) body += `\\bullet\\tab \\par
`;
      const rtf = `{\\rtf1\\ansi\\deff0{\\fonttbl{\\f0\\fswiss Helvetica;}}\\f0\\fs24
${body}}`;
      const blob = new Blob([rtf], { type: "application/rtf" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `Shopping List ${shoppingList.range.replace(/[/\\:]/g, "-")}.rtf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    };
    const addForm = /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, padding: 10, background: COLORS.greenSoft, borderRadius: 8 } }, /* @__PURE__ */ React.createElement(Select, { value: newRecipeType, onChange: (e) => setNewRecipeType(e.target.value), style: { marginBottom: 8 } }, /* @__PURE__ */ React.createElement("option", { value: "recipe" }, "Recipe"), /* @__PURE__ */ React.createElement("option", { value: "item" }, "Individual item")), /* @__PURE__ */ React.createElement(Input, { placeholder: newRecipeType === "item" ? "Item name" : "Recipe name", value: newRecipeName, onChange: (e) => setNewRecipeName(e.target.value), style: { width: "100%", marginBottom: 8 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "Servings (optional)", value: newServings, onChange: (e) => setNewServings(e.target.value), style: { width: 140, marginBottom: 8 } }), newIngredients.map((ing, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { display: "flex", gap: 6, marginBottom: 6 } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "qty", value: ing.qty, onChange: (e) => updateIngredientRow(i, "qty", e.target.value), style: { width: 55 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "unit", value: ing.unit, onChange: (e) => updateIngredientRow(i, "unit", e.target.value), style: { width: 65 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "ingredient", value: ing.name, onChange: (e) => updateIngredientRow(i, "name", e.target.value), style: { flex: 1 } }), /* @__PURE__ */ React.createElement("button", { onClick: () => removeIngredientRow(i), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"))), /* @__PURE__ */ React.createElement("button", { onClick: addIngredientRow, style: { background: "none", border: "none", color: COLORS.green, fontSize: 12, cursor: "pointer", padding: 0, marginBottom: 8 } }, "+ add ingredient"), /* @__PURE__ */ React.createElement(Input, { placeholder: "Instructions (optional)", value: newInstructions, onChange: (e) => setNewInstructions(e.target.value), style: { width: "100%", marginBottom: 8 } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 4 } }, "Nutrition per serving (optional)"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "kcal", value: newCalories, onChange: (e) => setNewCalories(e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "protein g", value: newProtein, onChange: (e) => setNewProtein(e.target.value), style: { width: 80 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "carbs g", value: newCarbs, onChange: (e) => setNewCarbs(e.target.value), style: { width: 80 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fat g", value: newFat, onChange: (e) => setNewFat(e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fibre g", value: newFiber, onChange: (e) => setNewFiber(e.target.value), style: { width: 75 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "calcium mg", value: newCalcium, onChange: (e) => setNewCalcium(e.target.value), style: { width: 85 } })), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, /* @__PURE__ */ React.createElement(Btn, { onClick: saveNewRecipe }, newRecipeType === "item" ? "Save item" : "Save recipe"), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: () => setShowAddRecipe(false) }, "Cancel")));
    return /* @__PURE__ */ React.createElement("div", null, retreat && /* @__PURE__ */ React.createElement(Card, { style: { borderColor: COLORS.gold, background: COLORS.claySoft } }, /* @__PURE__ */ React.createElement(Pill, { tone: "clay" }, "Retreat menu \u2014 ", retreat.theme), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 14, lineHeight: 1.7, color: COLORS.ink } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("b", null, "Breakfast:"), " ", RETREAT_CONSTANTS.breakfast), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("b", null, "Snack:"), " ", RETREAT_CONSTANTS.snack), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("b", null, "Lunch:"), " ", retreat.lunch), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("b", null, "Dinner:"), " ", retreat.dinner))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Plan a day"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: day, onChange: (e) => setDay(e.target.value), style: { marginBottom: 10 } }), /* @__PURE__ */ React.createElement("div", { style: { padding: 10, background: COLORS.greenSoft, borderRadius: 8, marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 6 } }, "Copy from another day"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "From"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: copyFromDate, onChange: (e) => setCopyFromDate(e.target.value) }), /* @__PURE__ */ React.createElement(Select, { value: copySourceSlot, onChange: (e) => setCopySourceSlot(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "breakfast" }, "Breakfast"), /* @__PURE__ */ React.createElement("option", { value: "lunch" }, "Lunch"), /* @__PURE__ */ React.createElement("option", { value: "dinner" }, "Dinner"), /* @__PURE__ */ React.createElement("option", { value: "snacks" }, "Snacks")), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "into"), /* @__PURE__ */ React.createElement(Select, { value: copyTargetSlot, onChange: (e) => setCopyTargetSlot(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "breakfast" }, "Breakfast"), /* @__PURE__ */ React.createElement("option", { value: "lunch" }, "Lunch"), /* @__PURE__ */ React.createElement("option", { value: "dinner" }, "Dinner"), /* @__PURE__ */ React.createElement("option", { value: "snacks" }, "Snacks")), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: copyMealSlot }, "Copy this meal")), /* @__PURE__ */ React.createElement(Btn, { onClick: copyEntireDay }, "Copy entire day"), copyMessage && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: COLORS.green, fontWeight: 600, marginTop: 6 } }, "\u2713 ", copyMessage), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginTop: 6 } }, "Copying replaces whatever's already in that slot (or the whole day) for ", fmtDate(day), ".")), [
      { slot: "breakfast", label: "Breakfast" },
      { slot: "lunch", label: "Lunch" },
      { slot: "dinner", label: "Dinner" },
      { slot: "snacks", label: "Snacks" }
    ].map(({ slot, label }) => {
      const items = getSlotItems(entry, slot);
      const slotTotalCal = items.reduce((sum, it) => {
        const r = it.recipeId ? recipes.find((rec) => rec.id === it.recipeId) : null;
        return sum + (r && r.calories != null ? r.calories * (it.servings || 1) : 0);
      }, 0);
      return /* @__PURE__ */ React.createElement("div", { key: slot, style: { marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginBottom: 3 } }, label), items.map((it) => {
        const recipe = it.recipeId ? recipes.find((r) => r.id === it.recipeId) : null;
        const lineCal = recipe && recipe.calories != null ? Math.round(recipe.calories * (it.servings || 1)) : null;
        return /* @__PURE__ */ React.createElement("div", { key: it.id, style: { display: "flex", alignItems: "center", gap: 6, padding: "4px 0" } }, /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 13.5, color: COLORS.ink } }, recipe ? recipe.name : it.customText), recipe && recipe.calories != null && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 3 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "\xD7"), /* @__PURE__ */ React.createElement(
          Input,
          {
            type: "number",
            min: "0.1",
            step: "0.5",
            value: it.servings === "" ? "" : it.servings ?? 1,
            onChange: (e) => {
              const raw = e.target.value;
              updateSlotItemServings(slot, it.id, raw === "" ? "" : parseFloat(raw));
            },
            onBlur: (e) => {
              if (e.target.value === "" || isNaN(parseFloat(e.target.value))) updateSlotItemServings(slot, it.id, 1);
            },
            style: { width: 55, fontSize: 12, padding: "4px 6px" }
          }
        )), lineCal != null && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.clay, fontWeight: 600, width: 62, textAlign: "right", flexShrink: 0 } }, lineCal, " kcal"), recipe && recipe.calories == null && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: COLORS.inkSoft } }, "no nutrition info"), /* @__PURE__ */ React.createElement("button", { onClick: () => removeSlotItem(slot, it.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"));
      }), slotTotalCal > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: COLORS.inkSoft, textAlign: "right", paddingTop: 2, paddingRight: 24 } }, label, " total: ", /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700, color: COLORS.ink } }, Math.round(slotTotalCal), " kcal")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 4 } }, /* @__PURE__ */ React.createElement(
        RecipeAutocomplete,
        {
          recipes,
          placeholder: `Add to ${label.toLowerCase()}\u2026`,
          onSelectRecipe: (recipeId) => addSlotItem(slot, { recipeId, servings: 1 }),
          onSelectCustom: (text) => addSlotItem(slot, { recipeId: null, customText: text })
        }
      )));
    }), /* @__PURE__ */ React.createElement(Input, { placeholder: "Notes", value: entry.notes, onChange: (e) => update("notes", e.target.value), style: { marginTop: 4 } })), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Nutrition"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 4 } }, "Daily targets"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "kcal", type: "number", value: nutritionTargets.calories ?? "", onChange: (e) => setTarget("calories", e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "protein g", type: "number", value: nutritionTargets.protein ?? "", onChange: (e) => setTarget("protein", e.target.value), style: { width: 85 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "carbs g", type: "number", value: nutritionTargets.carbs ?? "", onChange: (e) => setTarget("carbs", e.target.value), style: { width: 80 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fat g", type: "number", value: nutritionTargets.fat ?? "", onChange: (e) => setTarget("fat", e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fibre g", type: "number", value: nutritionTargets.fiber ?? "", onChange: (e) => setTarget("fiber", e.target.value), style: { width: 75 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "calcium mg", type: "number", value: nutritionTargets.calcium ?? "", onChange: (e) => setTarget("calcium", e.target.value), style: { width: 85 } })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 6 } }, "Quick log \u2014 for anything not tied to a saved recipe"), (entry.extraLogs || []).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 8 } }, entry.extraLogs.map((ex) => /* @__PURE__ */ React.createElement("div", { key: ex.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "4px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, fontSize: 13, color: COLORS.ink } }, ex.label), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, ex.calories, " kcal", ex.protein ? ` \xB7 ${ex.protein}p` : "", ex.carbs ? ` \xB7 ${ex.carbs}c` : "", ex.fat ? ` \xB7 ${ex.fat}f` : "", ex.fiber ? ` \xB7 ${ex.fiber}fibre` : "", ex.calcium ? ` \xB7 ${ex.calcium}mg calcium` : ""), /* @__PURE__ */ React.createElement("button", { onClick: () => removeQuickLog(ex.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715")))), /* @__PURE__ */ React.createElement(Input, { placeholder: "What did you have?", value: quickLogLabel, onChange: (e) => setQuickLogLabel(e.target.value), style: { width: "100%", marginBottom: 6 } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "kcal", type: "number", value: quickLogCalories, onChange: (e) => setQuickLogCalories(e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "protein g", type: "number", value: quickLogProtein, onChange: (e) => setQuickLogProtein(e.target.value), style: { width: 85 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "carbs g", type: "number", value: quickLogCarbs, onChange: (e) => setQuickLogCarbs(e.target.value), style: { width: 80 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fat g", type: "number", value: quickLogFat, onChange: (e) => setQuickLogFat(e.target.value), style: { width: 70 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "fibre g", type: "number", value: quickLogFiber, onChange: (e) => setQuickLogFiber(e.target.value), style: { width: 75 } }), /* @__PURE__ */ React.createElement(Input, { placeholder: "calcium mg", type: "number", value: quickLogCalcium, onChange: (e) => setQuickLogCalcium(e.target.value), style: { width: 85 } }), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: addQuickLog }, "Add")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 6 } }, "Totals for ", fmtDate(day)), [
      ...
      return /* @__PURE__ */ React.createElement("div", { key, style: { marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 12.5, color: COLORS.ink, marginBottom: 3 } }, /* @__PURE__ */ React.createElement("span", null, label), /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.inkSoft } }, value, unit, target ? ` / ${target}${unit}` : "")), target ? /* @__PURE__ */ React.createElement("div", { style: { height: 6, background: COLORS.line, borderRadius: 3, overflow: "hidden" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: `${pct}%`, background: pct > 100 ? COLORS.clay : COLORS.green } })) : null, remaining != null && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: remaining > 0 ? COLORS.clay : COLORS.green, marginTop: 3 } }, remaining > 0 ? `${remaining}${unit} left today` : remaining === 0 ? "Goal reached exactly" : `${Math.abs(remaining)}${unit} over goal`));
    }), dayFilledSlots > 0 && dayLoggedSlots < dayFilledSlots && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.clay, marginTop: 6 } }, dayLoggedSlots, " of ", dayFilledSlots, " planned meal", dayFilledSlots === 1 ? "" : "s", " today ", dayFilledSlots === 1 ? "has" : "have", " nutrition data \u2014 totals above are a partial picture. Custom-typed meals and recipes without nutrition info aren't counted."), dayFilledSlots === 0 && (entry.extraLogs || []).length === 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12.5, color: COLORS.inkSoft } }, "Nothing planned for this day yet.")), nutTrendData.length > 0 && /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Only counts days where at least one meal is linked to a recipe with nutrition info attached." }, "Nutrition trends"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 } }, [["30", "30 days"], ["90", "90 days"], ["365", "1 year"], ["all", "All time"], ["custom", "Custom"]].map(([val, label]) => /* @__PURE__ */ React.createElement(
      "button",
      {
        key: val,
      ...
      f.label
    ))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12.5, fontWeight: 700, color: COLORS.ink, marginBottom: 4 } }, selectedMacroField.label, " (", selectedMacroField.unit, ")"), /* @__PURE__ */ React.createElement("div", { style: { width: "100%", height: 200 } }, /* @__PURE__ */ React.createElement(ResponsiveContainer, null, /* @__PURE__ */ React.createElement(LineChart, { data: nutTrendData }, /* @__PURE__ */ React.createElement(CartesianGrid, { stroke: COLORS.line }), /* @__PURE__ */ React.createElement(XAxis, { dataKey: "date", fontSize: 11, stroke: COLORS.inkSoft }), /* @__PURE__ */ React.createElement(YAxis, { fontSize: 11, stroke: COLORS.inkSoft, domain: ["dataMin - 5", "dataMax + 5"] }), /* @__PURE__ */ React.createElement(Tooltip, null), nutritionTargets[selectedMacro] != null && /* @__PURE__ */ React.createElement(ReferenceLine, { y: nutritionTargets[selectedMacro], stroke: COLORS.gold, strokeDasharray: "4 4" }), /* @__PURE__ */ React.createElement(Line, { type: "monotone", dataKey: selectedMacro, stroke: COLORS.slate, strokeWidth: 2, dot: { r: 3 }, connectNulls: true }))))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("prelog"), onToggle: () => toggleSection("prelog") }, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "For anything you eat on repeat \u2014 set it once across several days at once. Each day stays fully editable afterward through 'Plan a day' above if your plan changes." }, "Pre-log for several days")), !isCollapsed("prelog") && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8, alignItems: "center" } }, /* @__PURE__ */ React.createElement(Select, { value: bulkSlot, onChange: (e) => setBulkSlot(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "breakfast" }, "Breakfast"), /* @__PURE__ */ React.createElement("option", { value: "lunch" }, "Lunch"), /* @__PURE__ */ React.createElement("option", { value: "dinner" }, "Dinner"), /* @__PURE__ */ React.createElement("option", { value: "snacks" }, "Snacks")), /* @__PURE__ */ React.createElement(
      RecipeAutocomplete,
      {
        recipes,
        placeholder: "Search recipes\u2026",
        onSelectRecipe: (id) => setBulkRecipeChoice(id),
        onSelectCustom: (text) => {
          setBulkRecipeChoice("__custom__");
          setBulkCustomText(text);
        }
      }
    ), bulkRecipeChoice !== "__custom__" && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 3 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "\xD7"), /* @__PURE__ */ React.createElement(Input, { type: "number", min: "0.1", step: "0.5", value: bulkServings, onChange: (e) => setBulkServings(parseFloat(e.target.value) || 1), style: { width: 55 } }))), bulkRecipeChoice !== "__custom__" && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12.5, color: COLORS.ink, marginBottom: 8 } }, "Selected: ", /* @__PURE__ */ React.createElement("b", null, (recipes.find((r) => r.id === bulkRecipeChoice) || {}).name), /* @__PURE__ */ React.createElement("button", { onClick: () => setBulkRecipeChoice("__custom__"), style: { marginLeft: 8, background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "clear")), /* @__PURE__ */ React.createElement(Input, { placeholder: "\u2026or type something custom", value: bulkRecipeChoice === "__custom__" ? bulkCustomText : "", onChange: (e) => {
      setBulkRecipeChoice("__custom__");
      setBulkCustomText(e.target.value);
    }, style: { width: "100%", marginBottom: 8 } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "Starting"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: bulkStartDate, onChange: (e) => setBulkStartDate(e.target.value) }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "for"), /* @__PURE__ */ React.createElement(Input, { type: "number", min: "1", value: bulkDays, onChange: (e) => setBulkDays(e.target.value), style: { width: 60 } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "day(s)"), /* @__PURE__ */ React.createElement(Btn, { onClick: applyBulkMealFill }, "Set")), bulkMessage && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12.5, color: COLORS.green, fontWeight: 600 } }, "\u2713 ", bulkMessage), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginTop: 6 } }, "This overwrites that slot on the selected days if something's already there."))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("items"), onToggle: () => toggleSection("items") }, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Single ingredients \u2014 chicken thigh, cherry tomatoes, avocado, etc. Add these to a meal slot on their own, or combine several into a recipe below." }, "Individual items")), !isCollapsed("items") && /* @__PURE__ */ React.createElement(React.Fragment, null, recipes.filter((r) => r.type === "item").length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.inkSoft, marginBottom: 10 } }, "No individual items yet.") : [...recipes].filter((r) => r.type === "item").sort((a, b) => a.name.localeCompare(b.name)).map(renderRecipeRow), showAddRecipe && newRecipeType === "item" ? addForm : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginTop: 10 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setNewRecipeType("item");
      setShowAddRecipe(true);
    }, style: { background: "none", border: "none", color: COLORS.green, fontSize: 12.5, fontWeight: 600, cursor: "pointer", padding: 0 } }, "+ add individual item")))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("recipes"), onToggle: () => toggleSection("recipes") }, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Add manually, or paste a recipe \u2014 name on the first line, then ingredients (either '2 cup flour' or 'Flour \u2013 2 cups', bulleted or not, with an optional Serves line and Ingredients/Method headers), then instructions. A 'Macros: 269 kcal | 30g protein | ...' summary line anywhere fills in the nutrition fields too." }, "Recipes")), !isCollapsed("recipes") && /* @__PURE__ */ React.createElement(React.Fragment, null, recipes.filter((r) => r.type !== "item").length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.inkSoft, marginBottom: 10 } }, "No recipes yet.") : [...recipes].filter((r) => r.type !== "item").sort((a, b) => a.name.localeCompare(b.name)).map(renderRecipeRow), !showAddRecipe || newRecipeType !== "recipe" ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginTop: 10 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => {
      setNewRecipeType("recipe");
      setShowAddRecipe(true);
    }, style: { background: "none", border: "none", color: COLORS.green, fontSize: 12.5, fontWeight: 600, cursor: "pointer", padding: 0 } }, "+ add recipe manually")) : addForm, /* @__PURE__ */ React.createElement("div", { style: { marginTop: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginBottom: 4 } }, "Or paste a recipe to parse automatically"), /* @__PURE__ */ React.createElement(
      "textarea",
      {
        value: pasteText,
      ...
      const nameGroups = Object.values(byName).sort((a, b) => a[0].name.localeCompare(b[0].name));
      return /* @__PURE__ */ React.createElement("div", { key: catName, style: { marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: COLORS.clay, textTransform: "uppercase", letterSpacing: 0.3, marginTop: 8, marginBottom: 2 } }, catName), nameGroups.map(
        (group) => group.length === 1 ? /* @__PURE__ */ React.createElement("div", { key: group[0].id, style: { display: "flex", alignItems: "center", gap: 8, padding: "5px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: group[0].done, onChange: () => toggleShoppingItem(group[0].id), style: { accentColor: COLORS.green, width: 15, height: 15 } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.ink, textDecoration: group[0].done ? "line-through" : "none", opacity: group[0].done ? 0.5 : 1 } }, group[0].qty, " ", group[0].unit, " ", group[0].name)) : /* @__PURE__ */ React.createElement("div", { key: group[0].name, style: { padding: "4px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, fontWeight: 600, color: COLORS.ink, marginBottom: 2 } }, group[0].name), group.map((i) => /* @__PURE__ */ React.createElement("div", { key: i.id, style: { display: "flex", alignItems: "center", gap: 8, paddingLeft: 14, padding: "2px 0 2px 14px" } }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: i.done, onChange: () => toggleShoppingItem(i.id), style: { accentColor: COLORS.green, width: 15, height: 15 } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: COLORS.inkSoft, textDecoration: i.done ? "line-through" : "none", opacity: i.done ? 0.5 : 1 } }, i.qty, " ", i.unit))))
      ));
    }), shoppingList.items.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: printShoppingList }, "\u{1F5A8} Print / Save as PDF"), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: downloadShoppingListWord }, "\u{1F4DD} Download as Word doc")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginTop: 4 } }, 'Tick whatever you actually want from this trip first \u2014 only ticked items go on either one. The Word doc adds a blank "My items" section at the end to type your usual extras into.')), shoppingList.unmatched && shoppingList.unmatched.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, fontWeight: 700, color: COLORS.clay, marginBottom: 4 } }, "Not itemised (no recipe linked) \u2014 plan for these separately:"), shoppingList.unmatched.map((u, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 12, color: COLORS.inkSoft, padding: "2px 0" } }, u))))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Logged days"), Object.keys(meals).sort().reverse().slice(0, 10).map((d) => /* @__PURE__ */ React.createElement("div", { key: d, style: { padding: "6px 0", borderBottom: `1px solid ${COLORS.line}`, fontSize: 13 } }, /* @__PURE__ */ React.createElement("div", { style: { color: COLORS.inkSoft, fontSize: 11.5 } }, fmtDate(d)), /* @__PURE__ */ React.createElement("div", null, [meals[d].breakfast, meals[d].lunch, meals[d].dinner, meals[d].snacks].filter(Boolean).join(" \xB7 "))))));
  }
  function TodoTab({ todos, setTodos, weeklyPool, setWeeklyPool, todayPicks, setTodayPicks, dismissedIds, setDismissedIds, recurringTasks, setRecurringTasks, focusItems, setFocusItems, focusDone, today }) {
    ...
    const addFocusItem = () => {
      ...
      setFocusItems([...focusItems, { id: "focus-" + Date.now(), text: newFocusText.trim() }]);
      setNewFocusText("");
    };
    const removeFocusItem = (id) => setFocusItems(focusItems.filter((f) => f.id !== id));
    const moveFocusItem = (id, direction) => {
      const idx = focusItems.findIndex((f) => f.id === id);
      const swapIdx = idx + direction;
      if (idx === -1 || swapIdx < 0 || swapIdx >= focusItems.length) return;
      const next = [...focusItems];
      [next[idx], next[swapIdx]] = [next[swapIdx], next[idx]];
      setFocusItems(next);
    };
    const [newFocusStepText, setNewFocusStepText] = useState({});
    const [editingFocusId, setEditingFocusId] = useState(null);
    const [editFocusText, setEditFocusText] = useState("");
    const startEditFocus = (f) => {
      setEditingFocusId(f.id);
      setEditFocusText(f.text);
    };
    const saveEditFocus = (id) => {
      if (editFocusText.trim()) setFocusItems(focusItems.map((f) => f.id === id ? { ...f, text: editFocusText.trim() } : f));
      setEditingFocusId(null);
    };
    const [expandedSteps, setExpandedSteps] = useState({});
    const toggleStepsExpanded = (focusId) => setExpandedSteps({ ...expandedSteps, [focusId]: !expandedSteps[focusId] });
    const addFocusStep = (focusId, text2) => {
      if (!text2.trim()) return;
      setFocusItems(focusItems.map((f) => f.id === focusId ? { ...f, steps: [...f.steps || [], { id: "fstep-" + Date.now(), text: text2.trim(), done: false }] } : f));
    };
    const toggleFocusStepDone = (focusId, stepId) => {
      setFocusItems(focusItems.map((f) => f.id === focusId ? { ...f, steps: (f.steps || []).map((s) => s.id === stepId ? { ...s, done: !s.done } : s) } : f));
    };
    const removeFocusStep = (focusId, stepId) => {
      setFocusItems(focusItems.map((f) => f.id === focusId ? { ...f, steps: (f.steps || []).filter((s) => s.id !== stepId) } : f));
    };
    const moveFocusStep = (focusId, stepId, direction) => {
      setFocusItems(focusItems.map((f) => {
        if (f.id !== focusId) return f;
        const steps = f.steps || [];
        const idx = steps.findIndex((s) => s.id === stepId);
        const swapIdx = idx + direction;
        if (idx === -1 || swapIdx < 0 || swapIdx >= steps.length) return f;
        const next = [...steps];
        [next[idx], next[swapIdx]] = [next[swapIdx], next[idx]];
        return { ...f, steps: next };
      }));
    };
    const [editingFocusStepId, setEditingFocusStepId] = useState(null);
    const [editFocusStepText, setEditFocusStepText] = useState("");
    const startEditFocusStep = (s) => {
      setEditingFocusStepId(s.id);
      setEditFocusStepText(s.text);
    };
    const saveEditFocusStep = (focusId, stepId) => {
      if (editFocusStepText.trim()) {
        setFocusItems(focusItems.map((f) => f.id === focusId ? { ...f, steps: (f.steps || []).map((s) => s.id === stepId ? { ...s, text: editFocusStepText.trim() } : s) } : f));
      }
      setEditingFocusStepId(null);
    };
    const dismiss = (id) => {
    ...
    const [expandedPoolSteps, setExpandedPoolSteps] = useState({});
    const [newPoolStepText, setNewPoolStepText] = useState({});
    const togglePoolStepsExpanded = (poolId) => setExpandedPoolSteps({ ...expandedPoolSteps, [poolId]: !expandedPoolSteps[poolId] });
    const addPoolStep = (poolId, text2) => {
      if (!text2.trim()) return;
      setWeeklyPool(weeklyPool.map((p) => p.id === poolId ? { ...p, steps: [...p.steps || [], { id: "pstep-" + Date.now(), text: text2.trim(), done: false }] } : p));
    };
    const togglePoolStepDone = (poolId, stepId) => {
      setWeeklyPool(weeklyPool.map((p) => p.id === poolId ? { ...p, steps: (p.steps || []).map((s) => s.id === stepId ? { ...s, done: !s.done } : s) } : p));
    };
    const removePoolStep = (poolId, stepId) => {
      setWeeklyPool(weeklyPool.map((p) => p.id === poolId ? { ...p, steps: (p.steps || []).filter((s) => s.id !== stepId) } : p));
    };
    const movePoolStep = (poolId, stepId, direction) => {
      setWeeklyPool(weeklyPool.map((p) => {
        if (p.id !== poolId) return p;
        const steps = p.steps || [];
        const idx = steps.findIndex((s) => s.id === stepId);
        const swapIdx = idx + direction;
        if (idx === -1 || swapIdx < 0 || swapIdx >= steps.length) return p;
        const next = [...steps];
        [next[idx], next[swapIdx]] = [next[swapIdx], next[idx]];
        return { ...p, steps: next };
      }));
    };
    const [editingPoolStepId, setEditingPoolStepId] = useState(null);
    const [editPoolStepText, setEditPoolStepText] = useState("");
    const startEditPoolStep = (s) => {
      setEditingPoolStepId(s.id);
      setEditPoolStepText(s.text);
    };
    const saveEditPoolStep = (poolId, stepId) => {
      if (editPoolStepText.trim()) {
        setWeeklyPool(weeklyPool.map((p) => p.id === poolId ? { ...p, steps: (p.steps || []).map((s) => s.id === stepId ? { ...s, text: editPoolStepText.trim() } : s) } : p));
      }
      setEditingPoolStepId(null);
    };
    const picksToday = todayPicks[today] || [];
    const togglePick = (id) => {
      const cur = todayPicks[today] || [];
      if (cur.includes(id)) {
        setTodayPicks({ ...todayPicks, [today]: cur.filter((i) => i !== id) });
        return;
      }
      const item = weeklyPool.find((p) => p.id === id);
      if (!item) return;
      setTodos([...todos, { id: "t-" + Date.now(), date: today, text: item.text, source: "manual", done: false }]);
      setWeeklyPool(weeklyPool.filter((p) => p.id !== id));
    };
    const renderPoolGroup = (items) => items.map((p, i) => {
      ...
      ) : /* @__PURE__ */ React.createElement("div", { style: { flex: 1, fontSize: 14, color: COLORS.ink, lineHeight: 1.4 } }, p.text)), !editing && /* @__PURE__ */ React.createElement("div", { style: { marginLeft: 32 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => togglePoolStepsExpanded(p.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11.5, padding: "2px 0" } }, (p.steps || []).length > 0 ? `${expandedPoolSteps[p.id] ? "\u25BE" : "\u25B8"} steps (${(p.steps || []).filter((s) => s.done).length}/${p.steps.length})` : `${expandedPoolSteps[p.id] ? "\u25BE" : "\u25B8"} add steps`), expandedPoolSteps[p.id] && /* @__PURE__ */ React.createElement("div", { style: { paddingLeft: 4 } }, (p.steps || []).map((s, si) => {
        const pSteps = p.steps || [];
        const stepEditing = editingPoolStepId === s.id;
        return /* @__PURE__ */ React.createElement("div", { key: s.id, style: { display: "flex", alignItems: "center", gap: 6, padding: "3px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => movePoolStep(p.id, s.id, -1), disabled: si === 0, style: { background: "none", border: "none", color: si === 0 ? COLORS.line : COLORS.inkSoft, cursor: si === 0 ? "default" : "pointer", fontSize: 10, lineHeight: 1, padding: "0 3px" } }, "\u25B2"), /* @__PURE__ */ React.createElement("button", { onClick: () => movePoolStep(p.id, s.id, 1), disabled: si === pSteps.length - 1, style: { background: "none", border: "none", color: si === pSteps.length - 1 ? COLORS.line : COLORS.inkSoft, cursor: si === pSteps.length - 1 ? "default" : "pointer", fontSize: 10, lineHeight: 1, padding: "0 3px" } }, "\u25BC")), /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: s.done, onChange: () => togglePoolStepDone(p.id, s.id), style: { accentColor: COLORS.clay, width: 14, height: 14, flexShrink: 0 } }), stepEditing ? /* @__PURE__ */ React.createElement(
          Input,
          {
            value: editPoolStepText,
            onChange: (e) => setEditPoolStepText(e.target.value),
            onKeyDown: (e) => {
              if (e.key === "Enter") saveEditPoolStep(p.id, s.id);
              if (e.key === "Escape") setEditingPoolStepId(null);
            },
            autoFocus: true,
            style: { flex: 1, fontSize: 12.5 }
          }
        ) : /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 13, color: s.done ? COLORS.inkSoft : COLORS.ink, textDecoration: s.done ? "line-through" : "none" } }, s.text), stepEditing ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => saveEditPoolStep(p.id, s.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11, fontWeight: 700 } }, "save"), /* @__PURE__ */ React.createElement("button", { onClick: () => setEditingPoolStepId(null), style: { background: "none", border: "none", color: COLORS.inkSoft, cursor: "pointer", fontSize: 11 } }, "cancel")) : /* @__PURE__ */ React.createElement("button", { onClick: () => startEditPoolStep(s), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11 } }, "edit"), /* @__PURE__ */ React.createElement("button", { onClick: () => removePoolStep(p.id, s.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 11 } }, "\u2715"));
      }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 4 } }, /* @__PURE__ */ React.createElement(
        Input,
        {
          placeholder: "Add a step",
          value: newPoolStepText[p.id] || "",
          onChange: (e) => setNewPoolStepText({ ...newPoolStepText, [p.id]: e.target.value }),
          onKeyDown: (e) => {
            if (e.key === "Enter") {
              addPoolStep(p.id, newPoolStepText[p.id] || "");
              setNewPoolStepText({ ...newPoolStepText, [p.id]: "" });
            }
          },
          style: { flex: 1, fontSize: 12.5 }
        }
    ...
    const startEditRecurring = (t) => {
      ...
      setNewRecurringText(t.text);
      setNewRecurringFrequency(t.frequency);
      if (t.frequency === "daily") setNewRecurringStartDate(t.startDate || todayISO());
      if (t.frequency === "weekly") setNewRecurringDayOfWeek(String(t.dayOfWeek));
      if (t.frequency === "monthly") setNewRecurringDayOfMonth(String(t.dayOfMonth));
      if (t.frequency === "interval-days") {
        setNewRecurringIntervalDays(String(t.intervalDays));
        setNewRecurringAnchorDate(t.anchorDate);
      }
      if (t.frequency === "interval-months") {
        setNewRecurringIntervalMonths(String(t.intervalMonths));
        setNewRecurringAnchorDate(t.anchorDate);
      }
      setNewRecurringEndDate(t.endDate || "");
    };
    ...
    const saveEditRecurring = () => {
      if (!newRecurringText.trim()) return;
      const t = { id: editingRecurringId, text: newRecurringText.trim(), frequency: newRecurringFrequency };
      if (newRecurringFrequency === "daily") {
        t.startDate = newRecurringStartDate || todayISO();
      } else if (newRecurringFrequency === "weekly") {
        t.dayOfWeek = parseInt(newRecurringDayOfWeek, 10);
      ...
      if (newRecurringEndDate) t.endDate = newRecurringEndDate;
      setRecurringTasks(recurringTasks.map((rt) => rt.id === editingRecurringId ? t : rt));
      cancelEditRecurring();
    };
    const [focusHistoryOpen, setFocusHistoryOpen] = useState(null);
    const [collapsedSections, setCollapsedSections] = useState({});
    const isCollapsed = (key) => collapsedSections[key] !== false;
    const toggleSection = (key) => setCollapsedSections({ ...collapsedSections, [key]: !isCollapsed(key) });
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Focus"), focusItems.map((f, i) => {
      const historyOpen = focusHistoryOpen === f.id;
      const editing = editingFocusId === f.id;
      const streak = computeFocusStreak(today, f.id, focusDone);
      const heatmap = focusHeatmapDays(today, f.id, focusDone, 35);
      return /* @__PURE__ */ React.createElement("div", { key: f.id, style: { padding: "6px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: editing ? "flex-start" : "center", gap: 8, flexWrap: editing ? "wrap" : "nowrap" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => moveFocusItem(f.id, -1), disabled: i === 0, style: { background: "none", border: "none", color: i === 0 ? COLORS.line : COLORS.inkSoft, cursor: i === 0 ? "default" : "pointer", fontSize: 11, lineHeight: 1, padding: "0 4px" } }, "\u25B2"), /* @__PURE__ */ React.createElement("button", { onClick: () => moveFocusItem(f.id, 1), disabled: i === focusItems.length - 1, style: { background: "none", border: "none", color: i === focusItems.length - 1 ? COLORS.line : COLORS.inkSoft, cursor: i === focusItems.length - 1 ? "default" : "pointer", fontSize: 11, lineHeight: 1, padding: "0 4px" } }, "\u25BC")), editing ? /* @__PURE__ */ React.createElement(
        "textarea",
        {
          value: editFocusText,
      ...
      ) : /* @__PURE__ */ React.createElement("div", { style: { flex: 1, fontSize: 14, color: COLORS.ink } }, f.text), streak > 1 && !editing && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.clay, fontWeight: 700 } }, "\u{1F525} ", streak), editing ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => saveEditFocus(f.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12, fontWeight: 700 } }, "save"), /* @__PURE__ */ React.createElement("button", { onClick: () => setEditingFocusId(null), style: { background: "none", border: "none", color: COLORS.inkSoft, cursor: "pointer", fontSize: 12 } }, "cancel")) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => startEditFocus(f), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11.5 } }, "edit"), /* @__PURE__ */ React.createElement("button", { onClick: () => setFocusHistoryOpen(historyOpen ? null : f.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11.5 } }, historyOpen ? "hide" : "history"), /* @__PURE__ */ React.createElement("button", { onClick: () => removeFocusItem(f.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"))), historyOpen && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, paddingLeft: 24 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: COLORS.inkSoft, marginBottom: 4 } }, "Last 35 days"), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 3, maxWidth: 220 } }, heatmap.map((h) => /* @__PURE__ */ React.createElement(
        "div",
        {
          key: h.date,
          title: `${fmtDate(h.date)}${h.done ? " \u2014 done" : ""}`,
          style: { width: "100%", aspectRatio: "1", borderRadius: 3, background: h.done ? COLORS.clay : COLORS.line }
        }
      )))), /* @__PURE__ */ React.createElement("div", { style: { marginLeft: 24, marginTop: 4 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => toggleStepsExpanded(f.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11.5, padding: "2px 0" } }, (f.steps || []).length > 0 ? `${expandedSteps[f.id] ? "\u25BE" : "\u25B8"} steps (${(f.steps || []).filter((s) => s.done).length}/${f.steps.length})` : `${expandedSteps[f.id] ? "\u25BE" : "\u25B8"} add steps`), expandedSteps[f.id] && /* @__PURE__ */ React.createElement("div", { style: { paddingLeft: 4 } }, (f.steps || []).map((s, si) => {
        const fSteps = f.steps || [];
        const stepEditing = editingFocusStepId === s.id;
        return /* @__PURE__ */ React.createElement("div", { key: s.id, style: { display: "flex", alignItems: "center", gap: 6, padding: "3px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => moveFocusStep(f.id, s.id, -1), disabled: si === 0, style: { background: "none", border: "none", color: si === 0 ? COLORS.line : COLORS.inkSoft, cursor: si === 0 ? "default" : "pointer", fontSize: 10, lineHeight: 1, padding: "0 3px" } }, "\u25B2"), /* @__PURE__ */ React.createElement("button", { onClick: () => moveFocusStep(f.id, s.id, 1), disabled: si === fSteps.length - 1, style: { background: "none", border: "none", color: si === fSteps.length - 1 ? COLORS.line : COLORS.inkSoft, cursor: si === fSteps.length - 1 ? "default" : "pointer", fontSize: 10, lineHeight: 1, padding: "0 3px" } }, "\u25BC")), /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: s.done, onChange: () => toggleFocusStepDone(f.id, s.id), style: { accentColor: COLORS.clay, width: 14, height: 14, flexShrink: 0 } }), stepEditing ? /* @__PURE__ */ React.createElement(
          Input,
          {
            value: editFocusStepText,
            onChange: (e) => setEditFocusStepText(e.target.value),
            onKeyDown: (e) => {
              if (e.key === "Enter") saveEditFocusStep(f.id, s.id);
              if (e.key === "Escape") setEditingFocusStepId(null);
            },
            autoFocus: true,
            style: { flex: 1, fontSize: 12.5 }
          }
        ) : /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 13, color: s.done ? COLORS.inkSoft : COLORS.ink, textDecoration: s.done ? "line-through" : "none" } }, s.text), stepEditing ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("button", { onClick: () => saveEditFocusStep(f.id, s.id), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11, fontWeight: 700 } }, "save"), /* @__PURE__ */ React.createElement("button", { onClick: () => setEditingFocusStepId(null), style: { background: "none", border: "none", color: COLORS.inkSoft, cursor: "pointer", fontSize: 11 } }, "cancel")) : /* @__PURE__ */ React.createElement("button", { onClick: () => startEditFocusStep(s), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 11 } }, "edit"), /* @__PURE__ */ React.createElement("button", { onClick: () => removeFocusStep(f.id, s.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 11 } }, "\u2715"));
      }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 4 } }, /* @__PURE__ */ React.createElement(
        Input,
        {
          placeholder: "Add a step",
          value: newFocusStepText[f.id] || "",
          onChange: (e) => setNewFocusStepText({ ...newFocusStepText, [f.id]: e.target.value }),
          onKeyDown: (e) => {
            if (e.key === "Enter") {
              addFocusStep(f.id, newFocusStepText[f.id] || "");
              setNewFocusStepText({ ...newFocusStepText, [f.id]: "" });
            }
          },
          style: { flex: 1, fontSize: 12.5 }
        }
    ...
    }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, marginTop: 10 } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "e.g. Less screen time", value: newFocusText, onChange: (e) => setNewFocusText(e.target.value), onKeyDown: (e) => {
      if (e.key === "Enter") addFocusItem();
    }, style: { flex: 1 } }), /* @__PURE__ */ React.createElement(Btn, { onClick: addFocusItem }, "Add"))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Weekly task pool"), picksToday.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: COLORS.inkSoft, marginBottom: 10 } }, picksToday.length, " picked for today \u2014 see them on Glance."), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Input, { placeholder: "Add a recurring/ongoing task", value: poolText, onChange: (e) => setPoolText(e.target.value), style: { flex: 1, minWidth: 140 } }), /* @__PURE__ */ React.createElement(Select, { value: poolCategory, onChange: (e) => setPoolCategory(e.target.value) }, POOL_CATEGORIES.map((c) => /* @__PURE__ */ React.createElement("option", { key: c.key, value: c.key }, c.label))), /* @__PURE__ */ React.createElement(Btn, { onClick: addPoolItem }, "Add")), POOL_CATEGORIES.map((c) => {
      const items = weeklyPool.filter((p) => categoryOf(p) === c.key && !picksToday.includes(p.id));
      const key = "pool-" + c.key;
      return /* @__PURE__ */ React.createElement("div", { key: c.key, style: { marginTop: 14 } }, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed(key), onToggle: () => toggleSection(key) }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12.5, fontWeight: 700, color: COLORS[c.color] } }, c.label), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginLeft: 6 } }, "(", items.length, ")")), !isCollapsed(key) && (items.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: COLORS.inkSoft, paddingTop: 6, paddingBottom: 4 } }, "Nothing here.") : /* @__PURE__ */ React.createElement("div", { style: { paddingTop: 4 } }, renderPoolGroup(items))));
    }), weeklyPool.filter((p) => categoryOf(p) === "other" && !picksToday.includes(p.id)).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 14 } }, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("pool-other"), onToggle: () => toggleSection("pool-other") }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12.5, fontWeight: 700, color: COLORS.inkSoft } }, "Uncategorised"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft, marginLeft: 6 } }, "(", weeklyPool.filter((p) => categoryOf(p) === "other" && !picksToday.includes(p.id)).length, ")")), !isCollapsed("pool-other") && /* @__PURE__ */ React.createElement("div", { style: { paddingTop: 4 } }, renderPoolGroup(weeklyPool.filter((p) => categoryOf(p) === "other" && !picksToday.includes(p.id)))))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("recurring"), onToggle: () => toggleSection("recurring") }, /* @__PURE__ */ React.createElement(SectionTitle, null, "Recurring tasks")), !isCollapsed("recurring") && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Input, { placeholder: "What's the task?", value: newRecurringText, onChange: (e) => setNewRecurringText(e.target.value), style: { width: "100%", marginBottom: 8 } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8, alignItems: "center" } }, /* @__PURE__ */ React.createElement(Select, { value: newRecurringFrequency, onChange: (e) => setNewRecurringFrequency(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "daily" }, "Every day"), /* @__PURE__ */ React.createElement("option", { value: "weekly" }, "Every week"), /* @__PURE__ */ React.createElement("option", { value: "monthly" }, "Every month"), /* @__PURE__ */ React.createElement("option", { value: "interval-days" }, "Every N days"), /* @__PURE__ */ React.createElement("option", { value: "interval-months" }, "Every N months")), newRecurringFrequency === "daily" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "starting"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: newRecurringStartDate, onChange: (e) => setNewRecurringStartDate(e.target.value) })), newRecurringFrequency === "weekly" && /* @__PURE__ */ React.createElement(Select, { value: newRecurringDayOfWeek, onChange: (e) => setNewRecurringDayOfWeek(e.target.value) }, WEEKDAY_NAMES.map((name, i) => /* @__PURE__ */ React.createElement("option", { key: i, value: i }, name))), newRecurringFrequency === "monthly" && /* @__PURE__ */ React.createElement(Input, { type: "number", min: "1", max: "31", placeholder: "Day of month", value: newRecurringDayOfMonth, onChange: (e) => setNewRecurringDayOfMonth(e.target.value), style: { width: 130 } }), newRecurringFrequency === "interval-days" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Input, { type: "number", min: "1", placeholder: "Every N days", value: newRecurringIntervalDays, onChange: (e) => setNewRecurringIntervalDays(e.target.value), style: { width: 130 } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "starting"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: newRecurringAnchorDate, onChange: (e) => setNewRecurringAnchorDate(e.target.value) })), newRecurringFrequency === "interval-months" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Input, { type: "number", min: "1", placeholder: "Every N months", value: newRecurringIntervalMonths, onChange: (e) => setNewRecurringIntervalMonths(e.target.value), style: { width: 130 } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "starting"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: newRecurringAnchorDate, onChange: (e) => setNewRecurringAnchorDate(e.target.value) }))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10, alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, "Ends (optional \u2014 leave blank to repeat indefinitely)"), /* @__PURE__ */ React.createElement(Input, { type: "date", value: newRecurringEndDate, onChange: (e) => setNewRecurringEndDate(e.target.value) }), editingRecurringId ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Btn, { onClick: saveEditRecurring }, "Save changes"), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: cancelEditRecurring }, "Cancel")) : /* @__PURE__ */ React.createElement(Btn, { onClick: addRecurringTask }, "Add")), recurringTasks.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: COLORS.inkSoft } }, "Nothing set up yet.") : recurringTasks.map((t) => {
      const editing = editingRecurringId === t.id;
      return /* @__PURE__ */ React.createElement("div", { key: t.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: `1px solid ${COLORS.line}`, flexWrap: "wrap", background: editing ? COLORS.claySoft : "transparent" } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 140 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: COLORS.ink } }, t.text), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11.5, color: COLORS.inkSoft } }, editing ? "Editing above \u2191" : describeRecurrence(t))), !editing && /* @__PURE__ */ React.createElement("button", { onClick: () => startEditRecurring(t), style: { background: "none", border: "none", color: COLORS.green, cursor: "pointer", fontSize: 12 } }, "edit"), /* @__PURE__ */ React.createElement("button", { onClick: () => removeRecurringTask(t.id), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"));
    }))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, "Add a dated to-do"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Input, { type: "date", value: date, onChange: (e) => setDate(e.target.value) }), /* @__PURE__ */ React.createElement(Input, { placeholder: "What needs doing", value: text, onChange: (e) => setText(e.target.value), style: { flex: 1, minWidth: 140 } }), /* @__PURE__ */ React.createElement(Btn, { onClick: add }, "Add"), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: addForTomorrow, disabled: !text }, "+ Tomorrow"))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("calendar"), onToggle: () => toggleSection("calendar") }, /* @__PURE__ */ React.createElement(SectionTitle, null, "Calendar")), !isCollapsed("calendar") && (upcoming.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: COLORS.inkSoft } }, "No synced calendar appointments yet.") : upcoming.map((t) => /* @__PURE__ */ React.createElement("div", { key: t.id, style: { padding: "8px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700, color: COLORS.slate } }, fmtDate(t.date)), /* @__PURE__ */ React.createElement("span", { style: { color: COLORS.ink } }, " \u2014 ", t.text))))));
  }
  function ScheduleSection({ viewDate, timeScheduleSettings, setTimeScheduleSettings, morningTemplate, setMorningTemplate, eveningTemplate, setEveningTemplate, timeSchedules, setTimeSchedules, todos, weeklyPool, recurringTasks }) {
    const dayEntry = timeSchedules[viewDate] || {};
    const hourMarks = generateHourMarks(timeScheduleSettings.dayStart, timeScheduleSettings.dayEnd);
    const calendarBlocks = calendarEventsForDate(viewDate, todos);
    const hourSplits = {};
    hourMarks.forEach((h) => {
      hourSplits[h] = computeHourSplit(h, dayEntry, morningTemplate, eveningTemplate, calendarBlocks);
    });
    const slots = hourMarks.flatMap((h) => getSlotsForHour(h, hourSplits[h]));
    const plannedMap = getEffectivePlanned(dayEntry, morningTemplate, eveningTemplate, calendarBlocks);
    const actualMap = dayEntry.actual || {};
    const taskOptions = [
      ...(todos || []).filter((t) => t.date === viewDate && t.source !== "calendar" && t.source !== "calendar-live" && t.source !== "retreat").map((t) => ({ id: "todo-" + t.id, text: t.text, groupLabel: "To-do list" })),
      ...(weeklyPool || []).map((p) => ({ id: "pool-" + p.id, text: p.text, groupLabel: poolCategoryLabel(p) })),
      ...dueRecurringTasksForDate(viewDate, recurringTasks).map((r) => ({ id: "recur-" + r.id, text: r.text, groupLabel: "Recurring tasks" }))
    ];
    const setBlock = (which, time, patch) => {
      const key = which === "planned" ? "planned" : "actual";
      const currentMap = which === "planned" ? plannedMap : actualMap;
      const nextMap = { ...currentMap, [time]: { ...currentMap[time] || {}, ...patch } };
      setTimeSchedules({ ...timeSchedules, [viewDate]: { ...dayEntry, [key]: nextMap } });
    };
    const setHourSplit = (hourMark, newSplit) => {
      const oldSlots = getSlotsForHour(hourMark, hourSplits[hourMark]);
      const newSlots = getSlotsForHour(hourMark, newSplit);
      const consolidate = (map) => {
        if (newSlots.length >= oldSlots.length) return map;
        const carry = oldSlots.map((s) => map[s]).find((b) => b && b.category);
        const next = { ...map };
        oldSlots.forEach((s) => delete next[s]);
        if (carry) next[newSlots[0]] = carry;
        return next;
      };
      const nextPlanned = consolidate(dayEntry.planned || {});
      const nextActual = consolidate(dayEntry.actual || {});
      setTimeSchedules({
        ...timeSchedules,
        [viewDate]: { ...dayEntry, planned: nextPlanned, actual: nextActual, hourSplits: { ...dayEntry.hourSplits || {}, [hourMark]: newSplit } }
      });
    };
    const markActualAsPlanned = (time) => {
      const planned = plannedMap[time];
      if (!planned) return;
      setBlock("actual", time, { category: planned.category, label: planned.label || "" });
    };
    const clearActual = (time) => setBlock("actual", time, { category: null, label: "" });
    const extendBlock = (which, time) => {
      const idx = slots.indexOf(time);
      if (idx === -1 || idx === slots.length - 1) return;
      const nextTime = slots[idx + 1];
      const currentMap = which === "planned" ? plannedMap : actualMap;
      const block = currentMap[time];
      if (!block) return;
      setBlock(which, nextTime, { category: block.category, label: block.label || "" });
    };
    const setNotes = (text) => setTimeSchedules({ ...timeSchedules, [viewDate]: { ...dayEntry, notes: text } });
    const [copyMsg, setCopyMsg] = useState("");
    const copySummary = async () => {
      const text = generateScheduleSummaryText(viewDate, plannedMap, actualMap, dayEntry.notes, slots);
      try {
        await navigator.clipboard.writeText(text);
        setCopyMsg("Copied \u2014 paste it into a chat with Claude whenever you want a look at it.");
      } catch (e) {
        setCopyMsg("Couldn't copy automatically \u2014 long-press the text area below to copy manually.");
      }
    };
    const [collapsedSections, setCollapsedSections] = useState({});
    const isCollapsed = (key) => collapsedSections[key] !== false;
    const toggleSection = (key) => setCollapsedSections({ ...collapsedSections, [key]: !isCollapsed(key) });
    const [openHourControl, setOpenHourControl] = useState(null);
    const splitLabel = (n) => n === 1 ? "1 hr" : n === 2 ? "30 min" : "15 min";
    const [newTemplateTime, setNewTemplateTime] = useState({ morning: "06:30", evening: "18:00" });
    const [newTemplateCategory, setNewTemplateCategory] = useState({ morning: "dogs", evening: "dogs" });
    const [newTemplateLabel, setNewTemplateLabel] = useState({ morning: "", evening: "" });
    const addTemplateBlock = (which) => {
      const setFn = which === "morning" ? setMorningTemplate : setEveningTemplate;
      const list = which === "morning" ? morningTemplate : eveningTemplate;
      const time = newTemplateTime[which];
      const category = newTemplateCategory[which];
      const label = newTemplateLabel[which];
      const next = [...list.filter((t) => t.time !== time), { time, category, label }].sort((a, b) => a.time.localeCompare(b.time));
      setFn(next);
      setNewTemplateLabel({ ...newTemplateLabel, [which]: "" });
    };
    const removeTemplateBlock = (which, time) => {
      const setFn = which === "morning" ? setMorningTemplate : setEveningTemplate;
      const list = which === "morning" ? morningTemplate : eveningTemplate;
      setFn(list.filter((t) => t.time !== time));
    };
    const renderTemplateEditor = (which, list) => /* @__PURE__ */ React.createElement(React.Fragment, null, list.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: COLORS.inkSoft, marginBottom: 8 } }, "No fixed blocks yet \u2014 add the parts of your ", which, " that are basically always the same.") : list.map((t) => {
      const cat = timeBlockCategoryOf(t.category);
      return /* @__PURE__ */ React.createElement("div", { key: t.time, style: { display: "flex", alignItems: "center", gap: 8, padding: "5px 0", borderBottom: `1px solid ${COLORS.line}` } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12.5, color: COLORS.inkSoft, width: 48, flexShrink: 0 } }, t.time), /* @__PURE__ */ React.createElement("span", { style: { width: 10, height: 10, borderRadius: 5, background: cat ? cat.color : COLORS.inkSoft, flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 13.5, color: COLORS.ink } }, t.label || (cat ? cat.label : t.category)), /* @__PURE__ */ React.createElement("button", { onClick: () => removeTemplateBlock(which, t.time), style: { background: "none", border: "none", color: COLORS.danger, cursor: "pointer", fontSize: 12 } }, "\u2715"));
    }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap", alignItems: "center" } }, /* @__PURE__ */ React.createElement(Input, { type: "time", value: newTemplateTime[which], onChange: (e) => setNewTemplateTime({ ...newTemplateTime, [which]: e.target.value }), style: { width: 100 } }), /* @__PURE__ */ React.createElement(Select, { value: newTemplateCategory[which], onChange: (e) => setNewTemplateCategory({ ...newTemplateCategory, [which]: e.target.value }) }, TIME_BLOCK_CATEGORIES.map((c) => /* @__PURE__ */ React.createElement("option", { key: c.key, value: c.key }, c.label))), /* @__PURE__ */ React.createElement(Input, { placeholder: "Label (optional)", value: newTemplateLabel[which], onChange: (e) => setNewTemplateLabel({ ...newTemplateLabel, [which]: e.target.value }), style: { flex: 1, minWidth: 120 } }), /* @__PURE__ */ React.createElement(Btn, { tone: "ghost", onClick: () => addTemplateBlock(which) }, "Add")));
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, null, fmtDate(viewDate)), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", fontSize: 11, color: COLORS.inkSoft, fontWeight: 700, marginBottom: 6, paddingLeft: 56 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 4 } }, "Planned"), /* @__PURE__ */ React.createElement("div", { style: { flex: 6 } }, "Actual")), hourMarks.map((hourMark) => {
      const split = hourSplits[hourMark];
      const hourSlots = getSlotsForHour(hourMark, split);
      return /* @__PURE__ */ React.createElement("div", { key: hourMark }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, padding: "2px 6px", marginTop: 2 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10.5, fontWeight: 700, color: COLORS.inkSoft, width: 48, flexShrink: 0 } }, hourMark), openHourControl === hourMark ? /* @__PURE__ */ React.createElement(
        Select,
        {
          value: split,
          onChange: (e) => {
            setHourSplit(hourMark, parseInt(e.target.value, 10));
            setOpenHourControl(null);
          },
          onBlur: () => setOpenHourControl(null),
          autoFocus: true,
          style: { fontSize: 10, padding: "1px 4px" }
        },
        /* @__PURE__ */ React.createElement("option", { value: 1 }, "1 hr"),
        /* @__PURE__ */ React.createElement("option", { value: 2 }, "30 min"),
        /* @__PURE__ */ React.createElement("option", { value: 4 }, "15 min")
      ) : /* @__PURE__ */ React.createElement("button", { onClick: () => setOpenHourControl(hourMark), style: { background: "none", border: "none", color: COLORS.inkSoft, fontSize: 10, cursor: "pointer", padding: 0 } }, splitLabel(split), " \u270E")), hourSlots.map((t) => {
        const planned = plannedMap[t];
        const actual = actualMap[t];
        const plannedCat = planned ? timeBlockCategoryOf(planned.category) : null;
        const actualCat = actual ? timeBlockCategoryOf(actual.category) : null;
        const bothSet = !!(planned && planned.category && actual && actual.category);
        const isMatch = bothSet && blocksMatch(planned, actual);
        const isMismatch = bothSet && !isMatch;
        const rowBg = isMatch ? "#A9D6A9" : isMismatch ? "#F0A99E" : "transparent";
        return /* @__PURE__ */ React.createElement("div", { key: t, style: { display: "flex", alignItems: "center", gap: 3, padding: "3px 4px", borderBottom: `1px solid ${COLORS.line}`, background: rowBg, borderRadius: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10.5, color: COLORS.inkSoft, width: 36, flexShrink: 0 } }, split === 1 ? "" : t), /* @__PURE__ */ React.createElement("div", { style: { flex: 3, minWidth: 0 } }, /* @__PURE__ */ React.createElement(
          BlockPicker,
          {
            value: planned,
            onSelect: (patch) => setBlock("planned", t, patch),
            taskOptions,
            colorHex: plannedCat ? plannedCat.color : null,
            placeholder: "\u2014"
          }
        )), planned && planned.category && /* @__PURE__ */ React.createElement("button", { onClick: () => extendBlock("planned", t), title: "Copy planned into the next slot", style: { background: COLORS.line, border: "none", borderRadius: 6, color: COLORS.ink, cursor: "pointer", fontSize: 13, fontWeight: 900, padding: "6px 7px", lineHeight: 1, flexShrink: 0 } }, "\u2193"), /* @__PURE__ */ React.createElement("div", { style: { flex: 3, minWidth: 0 } }, /* @__PURE__ */ React.createElement(
          BlockPicker,
          {
            value: actual,
            onSelect: (patch) => setBlock("actual", t, patch),
            taskOptions,
            colorHex: actualCat ? actualCat.color : null,
            placeholder: "What did you do?"
          }
        )), actual && actual.category && /* @__PURE__ */ React.createElement("button", { onClick: () => extendBlock("actual", t), title: "Copy actual into the next slot", style: { background: COLORS.line, border: "none", borderRadius: 6, color: COLORS.ink, cursor: "pointer", fontSize: 13, fontWeight: 900, padding: "6px 7px", lineHeight: 1, flexShrink: 0 } }, "\u2193"), /* @__PURE__ */ React.createElement("button", { onClick: () => markActualAsPlanned(t), disabled: !planned, title: "Did as planned", style: { background: planned ? COLORS.greenSoft : "none", border: "none", borderRadius: 6, color: planned ? COLORS.green : COLORS.line, cursor: planned ? "pointer" : "default", fontSize: 18, fontWeight: 900, padding: "6px 9px", lineHeight: 1, flexShrink: 0 } }, "\u2713"), /* @__PURE__ */ React.createElement("button", { onClick: () => clearActual(t), title: "Didn't do this \u2014 clear it", style: { background: COLORS.claySoft, border: "none", borderRadius: 6, color: COLORS.danger, cursor: "pointer", fontSize: 15, fontWeight: 900, padding: "6px 9px", lineHeight: 1, flexShrink: 0 } }, "\u2715"));
      }));
    })), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "A planned-vs-actual time log for the day, so you can see where time is actually going rather than just what got ticked off." }, "Schedule"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "Day runs from"), /* @__PURE__ */ React.createElement(Input, { type: "time", value: timeScheduleSettings.dayStart, onChange: (e) => setTimeScheduleSettings({ ...timeScheduleSettings, dayStart: e.target.value }), style: { width: 100 } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "to"), /* @__PURE__ */ React.createElement(Input, { type: "time", value: timeScheduleSettings.dayEnd, onChange: (e) => setTimeScheduleSettings({ ...timeScheduleSettings, dayEnd: e.target.value }), style: { width: 100 } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: COLORS.inkSoft } }, "\u2014 outside this is just Sleep."))), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("morning"), onToggle: () => toggleSection("morning") }, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "The fixed parts of your morning \u2014 these auto-fill into Planned on any day you haven't touched yet." }, "Morning template")), !isCollapsed("morning") && renderTemplateEditor("morning", morningTemplate)), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(CollapseHeader, { collapsed: isCollapsed("evening"), onToggle: () => toggleSection("evening") }, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Same idea for the fixed parts of your evening." }, "Evening template")), !isCollapsed("evening") && renderTemplateEditor("evening", eveningTemplate)), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Why did today go the way it did \u2014 helpful to look back on later." }, "Notes"), /* @__PURE__ */ React.createElement(
      "textarea",
      {
        value: dayEntry.notes || "",
        onChange: (e) => setNotes(e.target.value),
        placeholder: "What threw the day off (or kept it on track)?",
        rows: 3,
        style: { width: "100%", fontFamily: "inherit", fontSize: 14, padding: 10, borderRadius: 8, border: `1px solid ${COLORS.line}`, background: COLORS.card, color: COLORS.ink, resize: "vertical", boxSizing: "border-box" }
      }
    )), /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement(SectionTitle, { sub: "Copies Planned, Actual, and your notes as plain text \u2014 paste it into a chat with Claude for a look at how the day actually went." }, "Share for review"), /* @__PURE__ */ React.createElement(Btn, { onClick: copySummary }, "Copy day's schedule"), copyMsg && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12.5, color: COLORS.green, marginTop: 8 } }, copyMsg)));
  }
